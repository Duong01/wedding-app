<template>
  <div class="wedding-editor" :style="editorStyle">
    
    

    <EditorHeader
      :wedding="wedding"
      :routeTheme="routeTheme"
      :saving="saving"
      @back="backToTemplates"
      @preview="previewWedding"
      @save="saveWedding"
    />

    <!-- =====================================================
         LOADING
    ====================================================== -->
    <div v-if="loading" class="editor-loading">
      <div class="loading-card">
        <v-progress-circular indeterminate size="38" />

        <strong> Đang chuẩn bị trình chỉnh sửa... </strong>

        <span> Vui lòng chờ một chút </span>
      </div>
    </div>

    <!-- ERROR -->
    <div v-else-if="!wedding" class="editor-error">
      <div class="error-card">
        <div class="error-icon">
          <v-icon> mdi-alert-circle-outline </v-icon>
        </div>

        <h2>Không thể tải dữ liệu thiệp</h2>

        <p>
          {{ error || "Dữ liệu thiệp chưa sẵn sàng." }}
        </p>

        <button type="button" class="save-button" @click="initializeEditor">
          <v-icon> mdi-refresh </v-icon>

          Thử lại
        </button>
      </div>
    </div>

    <!-- =====================================================
         EDITOR
    ====================================================== -->
    <main v-else class="editor-layout">
      <!-- ===================================================
           SIDEBAR
      ==================================================== -->
      <aside class="editor-sidebar">
        <div class="sidebar-scroll">
          <div class="sidebar-section-title">THIỆP CƯỚI</div>

          <button
            v-for="item in menus"
            :key="item.id"
            type="button"
            class="editor-menu"
            :class="{
              active: activeMenu === item.id,
            }"
            @click="selectMenu(item.id)"
          >
            <span class="menu-icon">
              <v-icon size="18">
                {{ item.icon }}
              </v-icon>
            </span>

            <span class="menu-content">
              <strong>
                {{ item.label }}
              </strong>

              <small>
                {{ item.description }}
              </small>
            </span>
          </button>

          <div class="sidebar-divider"></div>

          <div class="sidebar-section-title">CẤU HÌNH</div>

          <button
            type="button"
            class="editor-menu"
            :class="{
              active: activeMenu === 'settings',
            }"
            @click="selectMenu('settings')"
          >
            <span class="menu-icon">
              <v-icon size="18"> mdi-tune-variant </v-icon>
            </span>

            <span class="menu-content">
              <strong>Cài đặt hiển thị</strong>

              <small> Bật / tắt nội dung </small>
            </span>
          </button>

          <button
            type="button"
            class="editor-menu"
            :class="{
              active: activeMenu === 'theme',
            }"
            @click="selectMenu('theme')"
          >
            <span class="menu-icon">
              <v-icon size="18"> mdi-palette-outline </v-icon>
            </span>

            <span class="menu-content">
              <strong>Giao diện</strong>

              <small> Màu sắc & font </small>
            </span>
          </button>
        </div>
      </aside>

      <!-- ===================================================
           CENTER EDITOR
      ==================================================== -->
      <section class="editor-center">
        <!-- =================================================
             GENERAL
        ================================================== -->
        <section v-if="activeMenu === 'general'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> WEDDING INFORMATION </span>

              <h1>Thông tin chung</h1>

              <p>Thông tin cơ bản của thiệp cưới.</p>
            </div>
          </div>

          <div class="form-grid">
            <div class="editor-field">
              <label>Tên chú rể</label>

              <input
                v-model="wedding.groomName"
                type="text"
                placeholder="Trần Hiếu"
              />
            </div>

            <div class="editor-field">
              <label>Tên cô dâu</label>

              <input
                v-model="wedding.brideName"
                type="text"
                placeholder="Hà Uyên"
              />
            </div>

            <div class="editor-field">
              <label>Slug</label>

              <input
                v-model="wedding.slug"
                type="text"
                placeholder="ha-uyen-tran-hieu"
              />

              <small class="field-help">
                Đường dẫn của thiệp trên website.
              </small>
            </div>

            <div class="editor-field">
              <label>Ngôn ngữ</label>

              <select v-model="wedding.language">
                <option value="vi">Tiếng Việt</option>

                <option value="en">English</option>
              </select>
            </div>

            <div class="editor-field">
              <label>Ngày cưới</label>

              <input v-model="wedding.weddingDate" type="datetime-local" />
            </div>

            <div class="editor-field full">
              <label>Ảnh bìa</label>

              <input
                v-model="wedding.coverImage"
                type="text"
                placeholder="https://..."
              />
            </div>
          </div>
        </section>

        <!-- =================================================
             COUPLE
        ================================================== -->
        <section v-if="activeMenu === 'couple'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> THE COUPLE </span>

              <h1>Cô dâu & Chú rể</h1>

              <p>Thông tin cá nhân của hai nhân vật chính.</p>
            </div>
          </div>

          <div class="person-grid">
            <!-- BRIDE -->
            <div class="person-editor">
              <div class="person-heading">
                <div class="person-avatar bride">
                  <v-icon> mdi-heart-outline </v-icon>
                </div>

                <div>
                  <span>CÔ DÂU</span>

                  <strong>
                    {{ wedding.couple?.Bride?.Name || "Cô dâu" }}
                  </strong>
                </div>
              </div>

              <div class="form-grid">
                <div class="editor-field">
                  <label>Họ tên</label>

                  <input v-model="wedding.couple.Bride.Name" type="text" />
                </div>

                <div class="editor-field">
                  <label>Tên gọi</label>

                  <input v-model="wedding.couple.Bride.Nickname" type="text" />
                </div>

                <div class="editor-field">
                  <label>Vai trò</label>

                  <input v-model="wedding.couple.Bride.Role" type="text" />
                </div>

                <div class="editor-field">
                  <label>Avatar</label>

                  <input v-model="wedding.couple.Bride.Avatar" type="text" />
                </div>

                <div class="editor-field full">
                  <label>Ảnh cover</label>

                  <input v-model="wedding.couple.Bride.Cover" type="text" />
                </div>

                <div class="editor-field">
                  <label>Cha</label>

                  <input v-model="wedding.couple.Bride.Father" type="text" />
                </div>

                <div class="editor-field">
                  <label>Mẹ</label>

                  <input v-model="wedding.couple.Bride.Mother" type="text" />
                </div>

                <div class="editor-field full">
                  <label>Địa chỉ</label>

                  <textarea v-model="wedding.couple.Bride.Address" rows="3" />
                </div>

                <div class="editor-field full">
                  <label>Giới thiệu</label>

                  <textarea
                    v-model="wedding.couple.Bride.Description"
                    rows="4"
                  />
                </div>
              </div>
            </div>

            <!-- GROOM -->
            <div class="person-editor">
              <div class="person-heading">
                <div class="person-avatar groom">
                  <v-icon> mdi-account-outline </v-icon>
                </div>

                <div>
                  <span>CHÚ RỂ</span>

                  <strong>
                    {{ wedding.couple?.Groom?.Name || "Chú rể" }}
                  </strong>
                </div>
              </div>

              <div class="form-grid">
                <div class="editor-field">
                  <label>Họ tên</label>

                  <input v-model="wedding.couple.Groom.Name" type="text" />
                </div>

                <div class="editor-field">
                  <label>Tên gọi</label>

                  <input v-model="wedding.couple.Groom.Nickname" type="text" />
                </div>

                <div class="editor-field">
                  <label>Vai trò</label>

                  <input v-model="wedding.couple.Groom.Role" type="text" />
                </div>

                <div class="editor-field">
                  <label>Avatar</label>

                  <input v-model="wedding.couple.Groom.Avatar" type="text" />
                </div>

                <div class="editor-field full">
                  <label>Ảnh cover</label>

                  <input v-model="wedding.couple.Groom.Cover" type="text" />
                </div>

                <div class="editor-field">
                  <label>Cha</label>

                  <input v-model="wedding.couple.Groom.Father" type="text" />
                </div>

                <div class="editor-field">
                  <label>Mẹ</label>

                  <input v-model="wedding.couple.Groom.Mother" type="text" />
                </div>

                <div class="editor-field full">
                  <label>Địa chỉ</label>

                  <textarea v-model="wedding.couple.Groom.Address" rows="3" />
                </div>

                <div class="editor-field full">
                  <label>Giới thiệu</label>

                  <textarea
                    v-model="wedding.couple.Groom.Description"
                    rows="4"
                  />
                </div>
              </div>
            </div>
          </div>
        </section>

        <!-- =================================================
             HERO
        ================================================== -->
        <section v-if="activeMenu === 'hero'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> HERO </span>

              <h1>Ảnh bìa</h1>

              <p>Nội dung xuất hiện đầu tiên khi khách mở thiệp.</p>
            </div>
          </div>

          <div class="form-grid">
            <div class="editor-field">
              <label>Tiêu đề</label>

              <input v-model="wedding.hero.Title" type="text" />
            </div>

            <div class="editor-field">
              <label>Lời mời</label>

              <input v-model="wedding.hero.Subtitle" type="text" />
            </div>

            <div class="editor-field">
              <label>Tên chú rể</label>

              <input v-model="wedding.hero.GroomName" type="text" />
            </div>

            <div class="editor-field">
              <label>Tên cô dâu</label>

              <input v-model="wedding.hero.BrideName" type="text" />
            </div>

            <div class="editor-field">
              <label>Ngày cưới</label>

              <input v-model="wedding.hero.WeddingDate" type="text" />
            </div>

            <div class="editor-field">
              <label>Địa điểm</label>

              <input v-model="wedding.hero.Location" type="text" />
            </div>

            <div class="editor-field full">
              <label>Ảnh nền</label>

              <input
                v-model="wedding.hero.Background"
                type="text"
                placeholder="/images/hero.jpg"
              />
            </div>

            <div class="editor-field full">
              <label>Nhạc</label>

              <input
                v-model="wedding.hero.Music"
                type="text"
                placeholder="/music/wedding.mp3"
              />
            </div>
          </div>
        </section>

        <!-- =================================================
             STORY
        ================================================== -->
        <section v-if="activeMenu === 'story'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> OUR STORY </span>

              <h1>Chuyện tình yêu</h1>

              <p>Kể lại câu chuyện của hai bạn.</p>
            </div>
          </div>

          <div class="editor-field">
            <label>Tiêu đề</label>

            <input v-model="wedding.story.Title" type="text" />
          </div>

          <div class="editor-field">
            <label>Nội dung</label>

            <textarea
              v-model="wedding.story.Description"
              rows="10"
              placeholder="Viết câu chuyện tình yêu..."
            />
          </div>
        </section>

        <!-- =================================================
             EVENTS
        ================================================== -->
        <section v-if="activeMenu === 'events'" class="editor-panel">
          <div class="panel-header panel-header-row">
            <div>
              <span class="panel-eyebrow"> WEDDING EVENTS </span>

              <h1>Sự kiện cưới</h1>

              <p>Các buổi lễ và tiệc cưới.</p>
            </div>

            <button
              type="button"
              class="small-primary-button"
              @click="addEvent"
            >
              <v-icon size="17"> mdi-plus </v-icon>

              Thêm sự kiện
            </button>
          </div>

          <div class="items-list">
            <article
              v-for="(event, index) in wedding.events"
              :key="event.Id || index"
              class="editor-card"
            >
              <div class="card-header">
                <div>
                  <span> SỰ KIỆN {{ index + 1 }} </span>

                  <strong>
                    {{ event.Title || "Chưa đặt tên" }}
                  </strong>
                </div>

                <button
                  type="button"
                  class="danger-icon"
                  title="Xóa sự kiện"
                  @click="removeEvent(index)"
                >
                  <v-icon size="18"> mdi-delete-outline </v-icon>
                </button>
              </div>

              <div class="form-grid">
                <div class="editor-field">
                  <label>Loại sự kiện</label>

                  <input v-model="event.EventType" type="text" />
                </div>

                <div class="editor-field">
                  <label>Tên sự kiện</label>

                  <input v-model="event.Title" type="text" />
                </div>

                <div class="editor-field">
                  <label>Thứ</label>

                  <input v-model="event.Weekday" type="text" />
                </div>

                <div class="editor-field">
                  <label>Ngày</label>

                  <input v-model="event.Day" type="text" />
                </div>

                <div class="editor-field">
                  <label>Tháng</label>

                  <input v-model="event.Month" type="text" />
                </div>

                <div class="editor-field">
                  <label>Năm</label>

                  <input v-model="event.Year" type="text" />
                </div>

                <div class="editor-field">
                  <label>Ngày tổ chức</label>

                  <input v-model="event.EventDate" type="date" />
                </div>

                <div class="editor-field">
                  <label>Giờ</label>

                  <input v-model="event.EventTime" type="time" />
                </div>

                <div class="editor-field">
                  <label>Âm lịch</label>

                  <input v-model="event.Lunar" type="text" />
                </div>

                <div class="editor-field">
                  <label>Địa điểm</label>

                  <input v-model="event.Location" type="text" />
                </div>

                <div class="editor-field full">
                  <label>Địa chỉ</label>

                  <input v-model="event.Address" type="text" />
                </div>

                <div class="editor-field full">
                  <label>Google Maps</label>

                  <input
                    v-model="event.Map"
                    type="text"
                    placeholder="https://maps.google.com/..."
                  />
                </div>
              </div>
            </article>

            <div v-if="!wedding.events?.length" class="empty-card">
              <v-icon size="32"> mdi-calendar-heart-outline </v-icon>

              <strong> Chưa có sự kiện </strong>

              <span> Hãy thêm sự kiện cưới đầu tiên. </span>
            </div>

            <button type="button" class="add-button" @click="addEvent">
              <v-icon> mdi-plus </v-icon>

              Thêm sự kiện
            </button>
          </div>
        </section>

        <!-- =================================================
             TIMELINE
        ================================================== -->
        <section v-if="activeMenu === 'timeline'" class="editor-panel">
          <div class="panel-header panel-header-row">
            <div>
              <span class="panel-eyebrow"> WEDDING DAY </span>

              <h1>Timeline</h1>

              <p>Các mốc thời gian trong ngày cưới.</p>
            </div>

            <button
              type="button"
              class="small-primary-button"
              @click="addTimeline"
            >
              <v-icon size="17"> mdi-plus </v-icon>

              Thêm mốc
            </button>
          </div>

          <div class="items-list">
            <article
              v-for="(item, index) in wedding.timeline"
              :key="item.Id || index"
              class="editor-card timeline-card"
            >
              <div class="card-header">
                <div>
                  <span> MỐC {{ index + 1 }} </span>

                  <strong>
                    {{ item.Title || "Chưa đặt tên" }}
                  </strong>
                </div>

                <button
                  type="button"
                  class="danger-icon"
                  title="Xóa mốc"
                  @click="removeTimeline(index)"
                >
                  <v-icon size="18"> mdi-delete-outline </v-icon>
                </button>
              </div>

              <div class="form-grid">
                <div class="editor-field">
                  <label>Thời gian</label>

                  <input v-model="item.Time" type="time" />
                </div>

                <div class="editor-field">
                  <label>Tiêu đề</label>

                  <input v-model="item.Title" type="text" />
                </div>

                <div class="editor-field">
                  <label>Địa điểm</label>

                  <input v-model="item.Location" type="text" />
                </div>

                <div class="editor-field">
                  <label>Icon</label>

                  <input v-model="item.Icon" type="text" placeholder="♡" />
                </div>

                <div class="editor-field full">
                  <label>Mô tả</label>

                  <textarea v-model="item.Description" rows="4" />
                </div>
              </div>
            </article>

            <button type="button" class="add-button" @click="addTimeline">
              <v-icon> mdi-plus </v-icon>

              Thêm mốc thời gian
            </button>
          </div>
        </section>

        <!-- =================================================
             GALLERY
        ================================================== -->
        <section v-if="activeMenu === 'gallery'" class="editor-panel">
          <div class="panel-header panel-header-row">
            <div>
              <span class="panel-eyebrow"> PHOTO ALBUM </span>

              <h1>Album ảnh</h1>

              <p>Thêm những khoảnh khắc đáng nhớ.</p>
            </div>

            <button
              type="button"
              class="small-primary-button"
              @click="addGallery"
            >
              <v-icon size="17"> mdi-image-plus-outline </v-icon>

              Thêm ảnh
            </button>
          </div>

          <div class="gallery-editor">
            <article
              v-for="(image, index) in wedding.gallery"
              :key="image.Id || index"
              class="gallery-item"
            >
              <img
                v-if="image.Image"
                :src="image.Image"
                :alt="`Ảnh ${index + 1}`"
                loading="lazy"
                @error="image.hasError = true"
              />

              <div v-else class="image-placeholder">
                <v-icon size="32"> mdi-image-outline </v-icon>

                <span> Chưa có ảnh </span>
              </div>

              <div class="gallery-input">
                <input
                  v-model="image.Image"
                  type="text"
                  placeholder="URL hình ảnh"
                />

                <button
                  type="button"
                  class="danger-icon"
                  title="Xóa ảnh"
                  @click="removeGallery(index)"
                >
                  <v-icon size="17"> mdi-delete-outline </v-icon>
                </button>
              </div>
            </article>
          </div>

          <button type="button" class="add-button" @click="addGallery">
            <v-icon> mdi-image-plus-outline </v-icon>

            Thêm ảnh
          </button>
        </section>

        <!-- =================================================
             RECIPIENT
        ================================================== -->
        <section v-if="activeMenu === 'recipient'" class="editor-panel">
          <div class="panel-header panel-header-row">
            <div>
              <span class="panel-eyebrow"> PERSONALIZATION </span>

              <h1>Khách mời</h1>

              <p>Tên khách được cá nhân hóa bằng token.</p>
            </div>

            <button
              type="button"
              class="small-primary-button"
              @click="addRecipient"
            >
              <v-icon size="17"> mdi-account-plus-outline </v-icon>

              Thêm khách
            </button>
          </div>

          <div class="items-list">
            <article
              v-for="(recipient, index) in wedding.recipientName"
              :key="recipient.token || index"
              class="editor-card"
            >
              <div class="card-header">
                <div>
                  <span> KHÁCH MỜI {{ index + 1 }} </span>

                  <strong>
                    {{ recipient.Name || "Khách mời" }}
                  </strong>
                </div>

                <button
                  type="button"
                  class="danger-icon"
                  title="Xóa khách"
                  @click="removeRecipient(index)"
                >
                  <v-icon size="18"> mdi-delete-outline </v-icon>
                </button>
              </div>

              <div class="form-grid">
                <div class="editor-field">
                  <label>Token</label>

                  <input v-model="recipient.token" type="text" />
                </div>

                <div class="editor-field">
                  <label>Tên khách</label>

                  <input v-model="recipient.Name" type="text" />
                </div>
              </div>
            </article>

            <button type="button" class="add-button" @click="addRecipient">
              <v-icon> mdi-account-plus-outline </v-icon>

              Thêm khách mời
            </button>
          </div>
        </section>

        <!-- =================================================
             GIFTS
        ================================================== -->
        <section v-if="activeMenu === 'gifts'" class="editor-panel">
          <div class="panel-header panel-header-row">
            <div>
              <span class="panel-eyebrow"> WEDDING GIFT </span>

              <h1>Mừng cưới</h1>

              <p>Thông tin tài khoản nhận mừng cưới.</p>
            </div>

            <button type="button" class="small-primary-button" @click="addGift">
              <v-icon size="17"> mdi-bank-plus </v-icon>

              Thêm phương thức
            </button>
          </div>

          <div class="items-list">
            <article
              v-for="(gift, index) in wedding.gifts"
              :key="gift.Id || index"
              class="editor-card"
            >
              <div class="card-header">
                <div>
                  <span> PHƯƠNG THỨC {{ index + 1 }} </span>

                  <strong>
                    {{ gift.Name || "Mừng cưới" }}
                  </strong>
                </div>

                <button
                  type="button"
                  class="danger-icon"
                  title="Xóa phương thức"
                  @click="removeGift(index)"
                >
                  <v-icon size="18"> mdi-delete-outline </v-icon>
                </button>
              </div>

              <div class="form-grid">
                <div class="editor-field">
                  <label>Tên</label>

                  <input v-model="gift.Name" type="text" />
                </div>

                <div class="editor-field">
                  <label>Ngân hàng / Ví</label>

                  <input v-model="gift.BankName" type="text" />
                </div>

                <div class="editor-field">
                  <label>Tên tài khoản</label>

                  <input v-model="gift.AccountName" type="text" />
                </div>

                <div class="editor-field">
                  <label>Số tài khoản</label>

                  <input v-model="gift.AccountNumber" type="text" />
                </div>

                <div class="editor-field full">
                  <label>QR Code</label>

                  <input
                    v-model="gift.QrCode"
                    type="text"
                    placeholder="/images/qr-bank.png"
                  />
                </div>

                <div class="editor-field full">
                  <label>Mô tả</label>

                  <textarea v-model="gift.Description" rows="4" />
                </div>
              </div>
            </article>

            <button type="button" class="add-button" @click="addGift">
              <v-icon> mdi-bank-plus </v-icon>

              Thêm phương thức
            </button>
          </div>
        </section>

        <!-- =================================================
             GUEST BOOK
        ================================================== -->
        <section v-if="activeMenu === 'guestbook'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> GUEST BOOK </span>

              <h1>Sổ lưu bút</h1>

              <p>Cho phép khách mời gửi lời chúc.</p>
            </div>
          </div>

          <div class="switch-card">
            <div>
              <strong> Hiển thị sổ lưu bút </strong>

              <small> Khách mời có thể gửi lời chúc. </small>
            </div>

            <v-switch
              v-model="wedding.guestBook.Enabled"
              color="primary"
              hide-details
            />
          </div>

          <div class="editor-field">
            <label>Tiêu đề</label>

            <input v-model="wedding.guestBook.Title" type="text" />
          </div>

          <div class="guest-list">
            <article
              v-for="guest in wedding.guestBook.Guest"
              :key="guest.Id"
              class="guest-card"
            >
              <div class="guest-avatar">
                <v-icon size="17"> mdi-account-heart-outline </v-icon>
              </div>

              <div class="guest-content">
                <strong>
                  {{ guest.Name }}
                </strong>

                <p>
                  {{ guest.Message }}
                </p>

                <small>
                  {{ guest.CreatedAt }}
                </small>
              </div>
            </article>

            <div v-if="!wedding.guestBook.Guest?.length" class="empty-card">
              <v-icon size="30"> mdi-message-heart-outline </v-icon>

              <strong> Chưa có lời chúc </strong>

              <span> Lời chúc của khách mời sẽ hiển thị ở đây. </span>
            </div>
          </div>
        </section>

        <!-- =================================================
             COUNTDOWN
        ================================================== -->
        <section v-if="activeMenu === 'countdown'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> COUNTDOWN </span>

              <h1>Đếm ngược</h1>

              <p>Hiển thị thời gian còn lại đến ngày cưới.</p>
            </div>
          </div>

          <div class="switch-card">
            <div>
              <strong> Hiển thị Countdown </strong>

              <small> Tự động đếm ngược đến ngày cưới. </small>
            </div>

            <v-switch
              v-model="wedding.countdown.Enabled"
              color="primary"
              hide-details
            />
          </div>

          <div class="editor-field">
            <label>Ngày kết thúc</label>

            <input v-model="wedding.countdown.Target" type="datetime-local" />
          </div>
        </section>

        <!-- =================================================
             MUSIC
        ================================================== -->
        <section v-if="activeMenu === 'music'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> WEDDING MUSIC </span>

              <h1>Âm nhạc</h1>

              <p>Thiết lập nhạc nền cho thiệp.</p>
            </div>
          </div>

          <div class="switch-card">
            <div>
              <strong> Bật nhạc </strong>

              <small> Hiển thị nút điều khiển nhạc. </small>
            </div>

            <v-switch
              v-model="wedding.music.Enabled"
              color="primary"
              hide-details
            />
          </div>

          <div class="switch-card">
            <div>
              <strong> Tự động phát </strong>

              <small> Trình duyệt có thể chặn autoplay. </small>
            </div>

            <v-switch
              v-model="wedding.music.Autoplay"
              color="primary"
              hide-details
            />
          </div>

          <div class="form-grid">
            <div class="editor-field">
              <label>Tên bài hát</label>

              <input v-model="wedding.music.Title" type="text" />
            </div>

            <div class="editor-field">
              <label>URL nhạc</label>

              <input
                v-model="wedding.music.Url"
                type="text"
                placeholder="/music/wedding.mp3"
              />
            </div>
          </div>
        </section>

        <!-- =================================================
             FOOTER
        ================================================== -->
        <section v-if="activeMenu === 'footer'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> FOOTER </span>

              <h1>Footer</h1>

              <p>Thông tin cuối thiệp.</p>
            </div>
          </div>

          <div class="form-grid">
            <div class="editor-field">
              <label>Tên chú rể</label>

              <input v-model="wedding.footer.GroomName" type="text" />
            </div>

            <div class="editor-field">
              <label>Tên cô dâu</label>

              <input v-model="wedding.footer.BrideName" type="text" />
            </div>

            <div class="editor-field">
              <label>Copyright</label>

              <input v-model="wedding.footer.Copyright" type="text" />
            </div>

            <div class="editor-field full">
              <label>Lời kết</label>

              <textarea v-model="wedding.footer.Message" rows="6" />
            </div>
          </div>
        </section>

        <!-- =================================================
             MAP
        ================================================== -->
        <section v-if="activeMenu === 'map'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> LOCATION </span>

              <h1>Bản đồ</h1>

              <p>Thiết lập địa điểm và Google Maps.</p>
            </div>
          </div>

          <div class="items-list">
            <article
              v-for="(event, index) in wedding.events"
              :key="event.Id || index"
              class="editor-card"
            >
              <div class="card-header">
                <div>
                  <span> ĐỊA ĐIỂM {{ index + 1 }} </span>

                  <strong>
                    {{ event.Title || "Sự kiện" }}
                  </strong>
                </div>
              </div>

              <div class="form-grid">
                <div class="editor-field full">
                  <label>Địa điểm</label>

                  <input v-model="event.Location" type="text" />
                </div>

                <div class="editor-field full">
                  <label>Địa chỉ</label>

                  <textarea v-model="event.Address" rows="3" />
                </div>

                <div class="editor-field full">
                  <label>Google Maps</label>

                  <input
                    v-model="event.Map"
                    type="text"
                    placeholder="https://maps.google.com/?q=..."
                  />
                </div>
              </div>
            </article>

            <div v-if="!wedding.events?.length" class="empty-card">
              <v-icon size="30"> mdi-map-marker-outline </v-icon>

              <strong> Chưa có địa điểm </strong>

              <span> Hãy thêm sự kiện trước khi thiết lập bản đồ. </span>
            </div>
          </div>
        </section>

        <!-- =================================================
             SETTINGS
        ================================================== -->
        <section v-if="activeMenu === 'settings'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> DISPLAY SETTINGS </span>

              <h1>Cài đặt hiển thị</h1>

              <p>Chọn những phần xuất hiện trên thiệp.</p>
            </div>
          </div>

          <div class="settings-grid">
            <div
              v-for="(value, key) in wedding.settings"
              :key="key"
              class="setting-item"
            >
              <div class="setting-info">
                <div class="setting-icon">
                  <v-icon size="17">
                    {{ settingIcon(key) }}
                  </v-icon>
                </div>

                <div>
                  <strong>
                    {{ settingLabel(key) }}
                  </strong>

                  <small>
                    {{ settingDescription(key) }}
                  </small>
                </div>
              </div>

              <v-switch
                v-model="wedding.settings[key]"
                color="primary"
                hide-details
              />
            </div>
          </div>
        </section>

        <!-- =================================================
             THEME
        ================================================== -->
        <section v-if="activeMenu === 'theme'" class="editor-panel">
          <div class="panel-header">
            <div>
              <span class="panel-eyebrow"> DESIGN SYSTEM </span>

              <h1>Giao diện</h1>

              <p>Tùy chỉnh màu sắc, font và bố cục.</p>
            </div>
          </div>

          <!-- CURRENT THEME -->
          <div class="theme-current">
            <div
              class="theme-color-preview"
              :style="{
                background: wedding.theme.Colors.Primary,
              }"
            >
              ♥
            </div>

            <div class="theme-current-info">
              <span> Theme hiện tại </span>

              <strong>
                {{ wedding.theme.Name }}
              </strong>
            </div>
          </div>

          <!-- COLORS -->
          <h3 class="sub-heading">Màu sắc</h3>

          <div class="color-grid">
            <div class="color-field">
              <label>Primary</label>

              <div class="color-control">
                <input v-model="wedding.theme.Colors.Primary" type="color" />

                <input v-model="wedding.theme.Colors.Primary" type="text" />
              </div>
            </div>

            <div class="color-field">
              <label>Secondary</label>

              <div class="color-control">
                <input v-model="wedding.theme.Colors.Secondary" type="color" />

                <input v-model="wedding.theme.Colors.Secondary" type="text" />
              </div>
            </div>

            <div class="color-field">
              <label>Accent</label>

              <div class="color-control">
                <input v-model="wedding.theme.Colors.Accent" type="color" />

                <input v-model="wedding.theme.Colors.Accent" type="text" />
              </div>
            </div>

            <div class="color-field">
              <label>Accent Light</label>

              <div class="color-control">
                <input
                  v-model="wedding.theme.Colors.AccentLight"
                  type="color"
                />

                <input v-model="wedding.theme.Colors.AccentLight" type="text" />
              </div>
            </div>

            <div class="color-field">
              <label>Background</label>

              <div class="color-control">
                <input v-model="wedding.theme.Colors.Background" type="color" />

                <input v-model="wedding.theme.Colors.Background" type="text" />
              </div>
            </div>

            <div class="color-field">
              <label>Background Secondary</label>

              <div class="color-control">
                <input
                  v-model="wedding.theme.Colors.BackgroundSecondary"
                  type="color"
                />

                <input
                  v-model="wedding.theme.Colors.BackgroundSecondary"
                  type="text"
                />
              </div>
            </div>

            <div class="color-field">
              <label>Text</label>

              <div class="color-control">
                <input v-model="wedding.theme.Colors.Text" type="color" />

                <input v-model="wedding.theme.Colors.Text" type="text" />
              </div>
            </div>

            <div class="color-field">
              <label>Text Secondary</label>

              <div class="color-control">
                <input
                  v-model="wedding.theme.Colors.TextSecondary"
                  type="color"
                />

                <input
                  v-model="wedding.theme.Colors.TextSecondary"
                  type="text"
                />
              </div>
            </div>

            <div class="color-field">
              <label>White</label>

              <div class="color-control">
                <input v-model="wedding.theme.Colors.White" type="color" />

                <input v-model="wedding.theme.Colors.White" type="text" />
              </div>
            </div>
          </div>

          <!-- FONT -->
          <h3 class="sub-heading">Font</h3>

          <div class="form-grid">
            <div class="editor-field">
              <label>Font nội dung</label>

              <input v-model="wedding.theme.Fonts.Main" type="text" />
            </div>

            <div class="editor-field">
              <label>Font tiêu đề</label>

              <input v-model="wedding.theme.Fonts.Heading" type="text" />
            </div>

            <div class="editor-field">
              <label>Font chữ nghệ thuật</label>

              <input v-model="wedding.theme.Fonts.Script" type="text" />
            </div>
          </div>

          <!-- LAYOUT -->
          <h3 class="sub-heading">Layout</h3>

          <div class="form-grid">
            <div class="editor-field">
              <label>Max width</label>

              <input v-model="wedding.theme.Layout.MaxWidth" type="text" />
            </div>

            <div class="editor-field">
              <label>Section padding</label>

              <input
                v-model="wedding.theme.Layout.SectionPadding"
                type="text"
              />
            </div>
          </div>
        </section>
      </section>

      <!-- ===================================================
           LIVE PREVIEW
      ==================================================== -->
      <aside class="editor-preview">
        <div class="preview-toolbar">
          <div class="preview-title">
            <span>LIVE PREVIEW</span>

            <strong>
              {{ wedding.theme.Name }}
            </strong>
          </div>

          <div class="preview-device">
            <button
              type="button"
              :class="{
                active: previewDevice === 'mobile',
              }"
              title="Xem trên điện thoại"
              @click="previewDevice = 'mobile'"
            >
              <v-icon size="18"> mdi-cellphone </v-icon>
            </button>

            <button
              type="button"
              :class="{
                active: previewDevice === 'desktop',
              }"
              title="Xem trên máy tính"
              @click="previewDevice = 'desktop'"
            >
              <v-icon size="18"> mdi-monitor </v-icon>
            </button>
          </div>
        </div>

        <div class="preview-stage" :class="`device-${previewDevice}`">
          <div class="preview-frame">
            <EditorPreview :wedding="wedding" />
          </div>
        </div>
      </aside>
    </main>

    <!-- =====================================================
         MOBILE BOTTOM BAR
    ====================================================== -->
    <div class="mobile-editor-actions">
      <button
        type="button"
        :class="{
          active: activeMenu === 'general',
        }"
        @click="selectMenu('general')"
      >
        <v-icon size="21"> mdi-information-outline </v-icon>

        <span> Thông tin </span>
      </button>

      <button
        type="button"
        :class="{
          active: mobileMenuOpen,
        }"
        @click="mobileMenuOpen = true"
      >
        <v-icon size="21"> mdi-view-list-outline </v-icon>

        <span> Nội dung </span>
      </button>

      <button type="button" @click="previewWedding">
        <v-icon size="21"> mdi-eye-outline </v-icon>

        <span> Xem </span>
      </button>

      <button type="button" :disabled="saving" @click="saveWedding">
        <v-progress-circular v-if="saving" indeterminate size="18" width="2" />

        <v-icon v-else size="21"> mdi-content-save-outline </v-icon>

        <span> Lưu </span>
      </button>
    </div>

    <!-- =====================================================
         MOBILE MENU SHEET
    ====================================================== -->
    <Transition name="sheet">
      <div
        v-if="mobileMenuOpen"
        class="mobile-menu-overlay"
        @click.self="mobileMenuOpen = false"
      >
        <div class="mobile-menu-sheet">
          <div class="sheet-handle"></div>

          <div class="sheet-header">
            <div>
              <span> WEDDING EDITOR </span>

              <strong> Chọn nội dung </strong>
            </div>

            <button
              type="button"
              class="sheet-close"
              @click="mobileMenuOpen = false"
            >
              <v-icon> mdi-close </v-icon>
            </button>
          </div>

          <div class="mobile-menu-list">
            <button
              v-for="item in menus"
              :key="item.id"
              type="button"
              class="mobile-menu-item"
              :class="{
                active: activeMenu === item.id,
              }"
              @click="selectMenu(item.id)"
            >
              <span class="mobile-menu-icon">
                <v-icon size="19">
                  {{ item.icon }}
                </v-icon>
              </span>

              <span>
                <strong>
                  {{ item.label }}
                </strong>

                <small>
                  {{ item.description }}
                </small>
              </span>

              <v-icon
                v-if="activeMenu === item.id"
                class="mobile-check"
                size="18"
              >
                mdi-check
              </v-icon>
            </button>

            <div class="mobile-menu-divider"></div>

            <button
              type="button"
              class="mobile-menu-item"
              :class="{
                active: activeMenu === 'settings',
              }"
              @click="selectMenu('settings')"
            >
              <span class="mobile-menu-icon">
                <v-icon size="19"> mdi-tune-variant </v-icon>
              </span>

              <span>
                <strong> Cài đặt hiển thị </strong>

                <small> Bật / tắt nội dung </small>
              </span>

              <v-icon
                v-if="activeMenu === 'settings'"
                class="mobile-check"
                size="18"
              >
                mdi-check
              </v-icon>
            </button>

            <button
              type="button"
              class="mobile-menu-item"
              :class="{
                active: activeMenu === 'theme',
              }"
              @click="selectMenu('theme')"
            >
              <span class="mobile-menu-icon">
                <v-icon size="19"> mdi-palette-outline </v-icon>
              </span>

              <span>
                <strong> Giao diện </strong>

                <small> Màu sắc & font </small>
              </span>

              <v-icon
                v-if="activeMenu === 'theme'"
                class="mobile-check"
                size="18"
              >
                mdi-check
              </v-icon>
            </button>
          </div>
        </div>
      </div>
    </Transition>

    <!-- =====================================================
         SAVE TOAST
    ====================================================== -->
    <Transition name="toast">
      <div
        v-if="saveMessage"
        class="save-toast"
        :class="{
          error: saveError,
        }"
      >
        <div class="toast-icon">
          <v-icon size="18">
            {{
              saveError
                ? "mdi-alert-circle-outline"
                : "mdi-check-circle-outline"
            }}
          </v-icon>
        </div>

        <span>
          {{ saveMessage }}
        </span>
      </div>
    </Transition>
  </div>
</template>

<script setup>
import { ref, computed, onMounted } from "vue";
import { useRouter, useRoute } from "vue-router";

import { useWeddingStore } from "@/stores/wedding";
import { useWeddingEditorStore } from "@/stores/weddingEditor";
import { AddDataWedding } from "@/model/api";
import EditorHeader from "@/components/editor/EditorHeader.vue";
import EditorPreview from "@/components/editor/EditorPreview.vue";

defineOptions({
  name: "WeddingEditor",
});

const router = useRouter();
const route = useRoute();

/* =========================================================
   STORE
========================================================= */

const weddingStore = useWeddingStore();
const editorStore = useWeddingEditorStore();

/* =========================================================
   STATE
========================================================= */

const activeMenu = ref("general");
const previewDevice = ref("desktop");
const mobileMenuOpen = ref(false);

const loading = ref(false);
const saving = ref(false);

const error = ref("");

const saveMessage = ref("");
const saveError = ref(false);

/* =========================================================
   ROUTE
========================================================= */

const routeTheme = computed(() => {
  const value = route.query.theme;

  if (typeof value === "string" && value.trim()) {
    return value.trim();
  }

  return "traditional-red";
});

const routeSlug = computed(() => {
  const value = route.query.slug;

  if (typeof value === "string" && value.trim()) {
    return value.trim();
  }

  return "";
});

/* =========================================================
   WEDDING
========================================================= */

const wedding = computed(() => {
  return editorStore.wedding;
});

/* =========================================================
   MENUS
========================================================= */

const menus = [
  {
    id: "general",
    label: "Thông tin chung",
    description: "Thông tin cơ bản",
    icon: "mdi-card-account-details-outline",
  },

  {
    id: "couple",
    label: "Cô dâu & Chú rể",
    description: "Thông tin hai bạn",
    icon: "mdi-heart-outline",
  },

  {
    id: "hero",
    label: "Ảnh bìa",
    description: "Màn hình mở đầu",
    icon: "mdi-image-outline",
  },

  {
    id: "story",
    label: "Chuyện tình yêu",
    description: "Câu chuyện của hai bạn",
    icon: "mdi-book-heart-outline",
  },

  {
    id: "events",
    label: "Sự kiện cưới",
    description: "Ngày giờ địa điểm",
    icon: "mdi-calendar-heart-outline",
  },

  {
    id: "timeline",
    label: "Timeline",
    description: "Lịch trình ngày cưới",
    icon: "mdi-timeline-outline",
  },

  {
    id: "gallery",
    label: "Album ảnh",
    description: "Khoảnh khắc đáng nhớ",
    icon: "mdi-image-multiple-outline",
  },

  {
    id: "recipient",
    label: "Khách mời",
    description: "Cá nhân hóa thiệp",
    icon: "mdi-account-multiple-outline",
  },

  {
    id: "gifts",
    label: "Mừng cưới",
    description: "Tài khoản nhận quà",
    icon: "mdi-gift-outline",
  },

  {
    id: "guestbook",
    label: "Sổ lưu bút",
    description: "Lời chúc khách mời",
    icon: "mdi-message-heart-outline",
  },
];

/* =========================================================
   SELECT MENU
========================================================= */

function selectMenu(id) {
  activeMenu.value = id;
  mobileMenuOpen.value = false;
}

/* =========================================================
   EVENT
========================================================= */

function addEvent() {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.events)) {
    wedding.value.events = [];
  }

  wedding.value.events.push({
    Id: Date.now(),
    EventType: "",
    Title: "",
    Weekday: "",
    Day: "",
    Month: "",
    Year: "",
    EventDate: "",
    EventTime: "",
    Lunar: "",
    Location: "",
    Address: "",
    Map: "",
  });
}

function removeEvent(index) {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.events)) return;

  wedding.value.events.splice(index, 1);
}

/* =========================================================
   TIMELINE
========================================================= */

function addTimeline() {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.timeline)) {
    wedding.value.timeline = [];
  }

  wedding.value.timeline.push({
    Id: Date.now(),
    Time: "",
    Title: "",
    Description: "",
    Location: "",
    Icon: "",
  });
}

function removeTimeline(index) {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.timeline)) return;

  wedding.value.timeline.splice(index, 1);
}

/* =========================================================
   GALLERY
========================================================= */

function addGallery() {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.gallery)) {
    wedding.value.gallery = [];
  }

  wedding.value.gallery.push({
    Id: Date.now(),
    Image: "",
  });
}

function removeGallery(index) {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.gallery)) return;

  wedding.value.gallery.splice(index, 1);
}

/* =========================================================
   RECIPIENT
========================================================= */

function addRecipient() {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.recipientName)) {
    wedding.value.recipientName = [];
  }

  wedding.value.recipientName.push({
    token: Math.random().toString(36).substring(2, 11),
    Name: "",
  });
}

function removeRecipient(index) {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.recipientName)) return;

  wedding.value.recipientName.splice(index, 1);
}

/* =========================================================
   GIFTS
========================================================= */

function addGift() {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.gifts)) {
    wedding.value.gifts = [];
  }

  wedding.value.gifts.push({
    Id: Date.now(),
    Name: "",
    Description: "",
    BankName: "",
    AccountName: "",
    AccountNumber: "",
    QrCode: "",
  });
}

function removeGift(index) {
  if (!wedding.value) return;

  if (!Array.isArray(wedding.value.gifts)) return;

  wedding.value.gifts.splice(index, 1);
}

/* =========================================================
   SAVE MESSAGE
========================================================= */

function showSaveMessage(message, isError = false) {
  saveMessage.value = message;
  saveError.value = isError;

  window.clearTimeout(showSaveMessage.timer);

  showSaveMessage.timer = window.setTimeout(() => {
    saveMessage.value = "";
    saveError.value = false;
  }, 2800);
}

/* =========================================================
   SAVE API
========================================================= */

function saveWedding() {
  if (!wedding.value || saving.value) {
    return;
  }

  saving.value = true;
  saveMessage.value = "";
  saveError.value = false;

  try {
    AddDataWedding(
      wedding.value,

      (result) => {
        console.log("[WeddingEditor] saved:", result);

        /*
         * Nếu API trả về dữ liệu mới
         * thì cập nhật lại Editor Store.
         */
        if (result && typeof result === "object") {
          try {
            editorStore.setWedding(result);
          } catch (e) {
            console.warn(
              "[WeddingEditor] Không thể cập nhật kết quả API:",
              e
            );
          }
        }

        showSaveMessage("Đã lưu thiệp thành công.");
        saving.value = false;
      },

      (err) => {
        console.error("[WeddingEditor] save error:", err);

        showSaveMessage(
          err?.message || "Không thể lưu thiệp.",
          true
        );

        saving.value = false;
      }
    );
  } catch (err) {
    console.error("[WeddingEditor] save exception:", err);

    showSaveMessage(
      err?.message || "Không thể lưu thiệp.",
      true
    );

    saving.value = false;
  }
}

/* =========================================================
   PREVIEW
========================================================= */

async function previewWedding() {
  if (!wedding.value) {
    console.warn(
      "[WeddingEditor] Không có wedding để preview."
    );

    return;
  }

  console.log(
    "[WeddingEditor] Preview:",
    wedding.value
  );

  /*
   * KHÔNG truyền wedding qua router.
   *
   * WeddingPreview sẽ lấy trực tiếp từ
   * useWeddingEditorStore().
   */
  await router.push({
    path: "/preview",
    query: {
      theme:
        wedding.value.theme?.Name ||
        routeTheme.value,
    },
  });
}

/* =========================================================
   BACK
========================================================= */

async function backToTemplates() {
  await router.push({
    path: "/templates",
  });
}

/* =========================================================
   INITIALIZE
========================================================= */

async function initializeEditor() {
  loading.value = true;
  error.value = "";

  try {
    /*
     * =====================================================
     * 1. ĐÃ CÓ DRAFT TRONG EDITOR STORE
     * =====================================================
     *
     * Đây là trường hợp:
     *
     * Editor
     *   ↓
     * Preview
     *   ↓
     * Editor
     *
     * Không được tạo lại dữ liệu.
     */
    if (editorStore.wedding) {
      console.log(
        "[WeddingEditor] Sử dụng wedding hiện tại:",
        editorStore.wedding
      );

      /*
       * Nếu URL có theme mới và wedding hiện tại
       * chưa có theme thì bổ sung.
       */
      if (
        !editorStore.wedding.theme ||
        !editorStore.wedding.theme.Name
      ) {
        editorStore.setTheme(routeTheme.value);
      }

      return;
    }

    /*
     * =====================================================
     * 2. EDITOR ĐƯỢC MỞ VỚI SLUG
     * =====================================================
     *
     * Ví dụ:
     *
     * /editor?slug=ha-uyen-tran-hieu
     *
     * Khi đó mới load API.
     */
    if (routeSlug.value) {
      console.log(
        "[WeddingEditor] Load API:",
        routeSlug.value
      );

      const result =
        await weddingStore.loadWedding(
          routeSlug.value
        );

      const data =
        result || weddingStore.wedding;

      if (!data) {
        throw new Error(
          "API không trả về dữ liệu thiệp."
        );
      }

      /*
       * Copy dữ liệu API.
       *
       * Editor Store sở hữu bản copy.
       */
      let copy;

      try {
        copy = structuredClone(data);
      } catch (cloneError) {
        console.warn(
          "[WeddingEditor] structuredClone failed:",
          cloneError
        );

        copy = JSON.parse(
          JSON.stringify(data)
        );
      }

      editorStore.setWedding(copy);

      console.log(
        "[WeddingEditor] API wedding:",
        editorStore.wedding
      );

      return;
    }

    /*
     * =====================================================
     * 3. EDITOR MỚI
     * =====================================================
     *
     * /editor?theme=traditional-red
     *
     * Không gọi API.
     *
     * Store tạo object rỗng.
     */
    console.log(
      "[WeddingEditor] Tạo thiệp mới:",
      routeTheme.value
    );

    editorStore.init(routeTheme.value);

    /*
     * Kiểm tra bắt buộc.
     */
    if (!editorStore.wedding) {
      throw new Error(
        "WeddingEditorStore không tạo được dữ liệu thiệp."
      );
    }

    console.log(
      "[WeddingEditor] New wedding:",
      editorStore.wedding
    );
  } catch (err) {
    console.error(
      "[WeddingEditor] initialize error:",
      err
    );

    error.value =
      err?.message ||
      "Không thể tải dữ liệu thiệp.";
  } finally {
    loading.value = false;
  }
}

/* =========================================================
   MOUNT
========================================================= */

onMounted(() => {
  initializeEditor();
});
</script>

<style >
/* =========================================================
   ROOT
========================================================= */

.wedding-editor {
  --editor-bg: #f4f5f7;
  --panel-bg: #ffffff;
  --border: #e6e8ec;
  --muted: #8c929c;
  --dark: #24262b;

  min-height: 100vh;

  background: radial-gradient(
      circle at top left,
      color-mix(in srgb, var(--primary) 3%, transparent),
      transparent 30%
    ),
    var(--editor-bg);

  color: var(--dark);

  font-family: Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;

  overflow: hidden;
}

/* =========================================================
   TOP BAR
========================================================= */

.editor-topbar {
  position: relative;

  z-index: 100;

  height: 68px;

  display: grid;

  grid-template-columns:
    minmax(220px, 1fr)
    auto
    minmax(220px, 1fr);

  align-items: center;

  gap: 20px;

  padding: 0 22px;

  background: rgba(255, 255, 255, 0.92);

  border-bottom: 1px solid var(--border);

  backdrop-filter: blur(20px);

  box-shadow: 0 1px 10px rgba(0, 0, 0, 0.03);
}

.editor-brand {
  display: flex;

  align-items: center;

  gap: 10px;

  min-width: 0;
}

.brand-logo {
  width: 38px;

  height: 38px;

  flex: 0 0 38px;

  border: 0;

  border-radius: 12px;

  display: flex;

  align-items: center;

  justify-content: center;

  cursor: pointer;

  color: #fff;

  font-size: 18px;

  background: linear-gradient(135deg, var(--primary), var(--accent));

  box-shadow: 0 7px 18px color-mix(in srgb, var(--primary) 20%, transparent);

  transition: transform 0.2s ease, box-shadow 0.2s ease;
}

.brand-logo:hover {
  transform: translateY(-1px) rotate(-3deg);

  box-shadow: 0 10px 24px color-mix(in srgb, var(--primary) 25%, transparent);
}

.brand-text {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.brand-text strong {
  font-size: 15px;

  font-weight: 750;

  letter-spacing: -0.01em;
}

.brand-text span {
  margin-top: 2px;

  font-size: 10px;

  color: var(--muted);

  letter-spacing: 0.08em;

  text-transform: uppercase;
}

.editor-template {
  display: flex;

  flex-direction: column;

  align-items: center;

  text-align: center;
}

.editor-template span {
  font-size: 9px;

  color: var(--muted);

  letter-spacing: 0.12em;

  text-transform: uppercase;
}

.editor-template strong {
  margin-top: 3px;

  color: var(--primary);

  font-size: 13px;

  font-weight: 700;
}

.editor-actions {
  display: flex;

  justify-content: flex-end;

  align-items: center;

  gap: 8px;

  min-width: 0;
}

.top-button,
.save-button {
  height: 38px;

  display: inline-flex;

  align-items: center;

  justify-content: center;

  gap: 7px;

  border-radius: 10px;

  padding: 0 13px;

  font-size: 12px;

  font-weight: 600;

  cursor: pointer;

  transition: transform 0.2s ease, background 0.2s ease, box-shadow 0.2s ease,
    border-color 0.2s ease;
}

.top-button {
  color: #4e535b;

  background: #fff;

  border: 1px solid var(--border);
}

.top-button:hover {
  background: #fafafa;

  border-color: #d9dce1;

  transform: translateY(-1px);
}

.save-button {
  color: #fff;

  background: var(--primary);

  border: 1px solid var(--primary);

  box-shadow: 0 7px 18px color-mix(in srgb, var(--primary) 18%, transparent);
}

.save-button:hover:not(:disabled) {
  transform: translateY(-1px);

  box-shadow: 0 10px 24px color-mix(in srgb, var(--primary) 24%, transparent);
}

.save-button:disabled {
  opacity: 0.65;

  cursor: not-allowed;
}

/* =========================================================
   LOADING / ERROR
========================================================= */

.editor-loading,
.editor-error {
  min-height: calc(100vh - 68px);

  display: flex;

  align-items: center;

  justify-content: center;

  padding: 30px;
}

.loading-card,
.error-card {
  width: min(420px, 100%);

  padding: 38px 28px;

  display: flex;

  flex-direction: column;

  align-items: center;

  text-align: center;

  gap: 10px;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 22px;

  box-shadow: 0 20px 60px rgba(0, 0, 0, 0.07);
}

.loading-card strong,
.error-card h2 {
  margin: 10px 0 0;

  font-size: 18px;
}

.loading-card span,
.error-card p {
  margin: 0;

  color: var(--muted);

  font-size: 13px;

  line-height: 1.6;
}

.error-icon {
  width: 56px;

  height: 56px;

  display: flex;

  align-items: center;

  justify-content: center;

  border-radius: 16px;

  color: #c62828;

  background: #fff1f1;
}

/* =========================================================
   MAIN LAYOUT
========================================================= */

.editor-layout {
  height: calc(100vh - 68px);

  display: grid;

  grid-template-columns:
    245px
    minmax(0, 1fr)
    minmax(360px, 43vw);

  min-width: 0;
}

/* =========================================================
   SIDEBAR
========================================================= */

.editor-sidebar {
  min-width: 0;

  overflow: hidden;

  background: #fff;

  border-right: 1px solid var(--border);
}

.sidebar-scroll {
  height: 100%;

  overflow-y: auto;

  padding: 15px 10px 25px;

  scrollbar-width: thin;
}

.sidebar-section-title {
  padding: 9px 12px;

  color: #a0a5ad;

  font-size: 9px;

  font-weight: 750;

  letter-spacing: 0.13em;
}

.editor-menu {
  width: 100%;

  display: flex;

  align-items: center;

  gap: 10px;

  padding: 8px 10px;

  margin: 2px 0;

  border: 0;

  border-radius: 11px;

  background: transparent;

  color: #555b64;

  text-align: left;

  cursor: pointer;

  transition: background 0.2s ease, color 0.2s ease, transform 0.2s ease;
}

.editor-menu:hover {
  background: #f7f7f8;

  transform: translateX(1px);
}

.editor-menu.active {
  color: var(--primary);

  background: color-mix(in srgb, var(--primary) 7%, #fff);
}

.menu-icon {
  width: 34px;

  height: 34px;

  flex: 0 0 34px;

  display: flex;

  align-items: center;

  justify-content: center;

  border-radius: 9px;

  background: #f7f7f8;

  color: #737983;

  transition: background 0.2s ease, color 0.2s ease;
}

.editor-menu.active .menu-icon {
  color: var(--primary);

  background: color-mix(in srgb, var(--primary) 10%, #fff);
}

.menu-content {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.menu-content strong {
  overflow: hidden;

  white-space: nowrap;

  text-overflow: ellipsis;

  font-size: 11.5px;

  font-weight: 650;
}

.menu-content small {
  margin-top: 2px;

  overflow: hidden;

  white-space: nowrap;

  text-overflow: ellipsis;

  color: #a0a4ab;

  font-size: 9.5px;
}

.sidebar-divider {
  height: 1px;

  margin: 14px 5px;

  background: var(--border);
}

/* =========================================================
   CENTER
========================================================= */

.editor-center {
  min-width: 0;

  overflow-x: hidden;

  overflow-y: auto;

  padding: 32px 30px 80px;

  scrollbar-width: thin;
}

.editor-panel {
  width: 100%;

  max-width: 820px;

  margin: 0 auto 40px;
}

.panel-header {
  margin-bottom: 28px;
}

.panel-header-row {
  display: flex;

  align-items: flex-end;

  justify-content: space-between;

  gap: 20px;
}

.panel-eyebrow {
  display: inline-block;

  margin-bottom: 7px;

  color: var(--primary);

  font-size: 9px;

  font-weight: 750;

  letter-spacing: 0.15em;

  text-transform: uppercase;
}

.panel-header h1 {
  margin: 0;

  color: #25272c;

  font-family: var(--font-heading), Georgia, serif;

  font-size: 31px;

  font-weight: 600;

  line-height: 1.1;

  letter-spacing: -0.02em;
}

.panel-header p {
  margin: 8px 0 0;

  color: #8d929a;

  font-size: 12px;

  line-height: 1.6;
}

/* =========================================================
   FORM
========================================================= */

.form-grid {
  display: grid;

  grid-template-columns: repeat(2, minmax(0, 1fr));

  gap: 17px;
}

.editor-field {
  min-width: 0;

  display: flex;

  flex-direction: column;

  gap: 7px;

  margin-bottom: 16px;
}

.form-grid .editor-field {
  margin-bottom: 0;
}

.editor-field.full {
  grid-column: 1 / -1;
}

.editor-field label,
.color-field label {
  color: #565b63;

  font-size: 11px;

  font-weight: 650;
}

.editor-field input,
.editor-field textarea,
.editor-field select,
.color-control input[type="text"] {
  width: 100%;

  box-sizing: border-box;

  min-width: 0;

  outline: none;

  border: 1px solid #dedfe3;

  border-radius: 10px;

  background: #fff;

  color: #292c31;

  padding: 11px 12px;

  font-family: inherit;

  font-size: 13px;

  line-height: 1.45;

  transition: border-color 0.2s ease, box-shadow 0.2s ease, background 0.2s ease;
}

.editor-field input::placeholder,
.editor-field textarea::placeholder {
  color: #b3b7bd;
}

.editor-field input:hover,
.editor-field textarea:hover,
.editor-field select:hover {
  border-color: #cfd2d7;
}

.editor-field input:focus,
.editor-field textarea:focus,
.editor-field select:focus {
  border-color: var(--primary);

  box-shadow: 0 0 0 3px color-mix(in srgb, var(--primary) 9%, transparent);
}

.editor-field textarea {
  resize: vertical;

  min-height: 80px;
}

.field-help {
  color: #a0a5ac;

  font-size: 10px;

  line-height: 1.4;
}

/* =========================================================
   PERSON
========================================================= */

.person-grid {
  display: grid;

  grid-template-columns: repeat(2, minmax(0, 1fr));

  gap: 20px;
}

.person-editor {
  min-width: 0;

  padding: 21px;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 18px;

  box-shadow: 0 8px 28px rgba(0, 0, 0, 0.035);
}

.person-heading {
  display: flex;

  align-items: center;

  gap: 12px;

  margin-bottom: 21px;

  padding-bottom: 17px;

  border-bottom: 1px solid #eff0f2;
}

.person-avatar {
  width: 42px;

  height: 42px;

  flex: 0 0 42px;

  display: flex;

  align-items: center;

  justify-content: center;

  border-radius: 13px;
}

.person-avatar.bride {
  color: #b75a76;

  background: #fdf0f4;
}

.person-avatar.groom {
  color: #667da9;

  background: #eef2fa;
}

.person-heading div:last-child {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.person-heading span {
  color: #a0a5ac;

  font-size: 8px;

  font-weight: 750;

  letter-spacing: 0.12em;
}

.person-heading strong {
  margin-top: 3px;

  overflow: hidden;

  white-space: nowrap;

  text-overflow: ellipsis;

  color: #34373d;

  font-family: var(--font-heading), Georgia, serif;

  font-size: 20px;

  font-weight: 600;
}

/* =========================================================
   CARDS
========================================================= */

.items-list {
  display: flex;

  flex-direction: column;

  gap: 14px;
}

.editor-card {
  padding: 19px;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 16px;

  box-shadow: 0 6px 24px rgba(0, 0, 0, 0.03);
}

.card-header {
  display: flex;

  align-items: center;

  justify-content: space-between;

  gap: 15px;

  margin-bottom: 19px;
}

.card-header > div {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.card-header span {
  color: #a0a5ad;

  font-size: 8px;

  font-weight: 750;

  letter-spacing: 0.1em;
}

.card-header strong {
  margin-top: 4px;

  overflow: hidden;

  white-space: nowrap;

  text-overflow: ellipsis;

  color: #30333a;

  font-size: 14px;

  font-weight: 700;
}

.danger-icon {
  width: 34px;

  height: 34px;

  flex: 0 0 34px;

  display: flex;

  align-items: center;

  justify-content: center;

  border: 0;

  border-radius: 9px;

  background: #fff2f2;

  color: #c62828;

  cursor: pointer;

  transition: transform 0.2s ease, background 0.2s ease;
}

.danger-icon:hover {
  background: #ffe3e3;

  transform: translateY(-1px);
}

.small-primary-button {
  height: 37px;

  flex: 0 0 auto;

  display: inline-flex;

  align-items: center;

  justify-content: center;

  gap: 6px;

  padding: 0 12px;

  border: 0;

  border-radius: 9px;

  background: var(--primary);

  color: #fff;

  font-size: 11px;

  font-weight: 650;

  cursor: pointer;

  box-shadow: 0 5px 15px color-mix(in srgb, var(--primary) 15%, transparent);

  transition: transform 0.2s ease, box-shadow 0.2s ease;
}

.small-primary-button:hover {
  transform: translateY(-1px);

  box-shadow: 0 8px 20px color-mix(in srgb, var(--primary) 21%, transparent);
}

.add-button {
  min-height: 45px;

  width: 100%;

  display: flex;

  align-items: center;

  justify-content: center;

  gap: 7px;

  border: 1px dashed #cfd2d7;

  border-radius: 11px;

  background: #fff;

  color: var(--primary);

  font-size: 12px;

  font-weight: 650;

  cursor: pointer;

  transition: border-color 0.2s ease, background 0.2s ease, transform 0.2s ease;
}

.add-button:hover {
  border-color: var(--primary);

  background: color-mix(in srgb, var(--primary) 3%, #fff);

  transform: translateY(-1px);
}

.empty-card {
  min-height: 170px;

  display: flex;

  flex-direction: column;

  align-items: center;

  justify-content: center;

  gap: 7px;

  padding: 25px;

  border: 1px dashed #d9dce0;

  border-radius: 15px;

  background: rgba(255, 255, 255, 0.6);

  color: #999;
}

.empty-card strong {
  color: #666;

  font-size: 13px;
}

.empty-card span {
  color: #a5a9ae;

  font-size: 11px;
}

/* =========================================================
   GALLERY
========================================================= */

.gallery-editor {
  display: grid;

  grid-template-columns: repeat(3, minmax(0, 1fr));

  gap: 14px;

  margin-bottom: 15px;
}

.gallery-item {
  min-width: 0;

  overflow: hidden;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 14px;

  box-shadow: 0 5px 20px rgba(0, 0, 0, 0.03);
}

.gallery-item img,
.image-placeholder {
  display: block;

  width: 100%;

  aspect-ratio: 1;

  object-fit: cover;
}

.image-placeholder {
  display: flex;

  flex-direction: column;

  align-items: center;

  justify-content: center;

  gap: 5px;

  color: #a7abb1;

  background: #f5f6f7;
}

.image-placeholder span {
  font-size: 10px;
}

.gallery-input {
  display: flex;

  gap: 6px;

  padding: 8px;
}

.gallery-input input {
  width: 100%;

  min-width: 0;

  border: 1px solid #dddfe3;

  border-radius: 8px;

  padding: 8px;

  outline: none;

  font-family: inherit;

  font-size: 10px;
}

.gallery-input input:focus {
  border-color: var(--primary);
}

/* =========================================================
   SWITCH
========================================================= */

.switch-card {
  min-height: 66px;

  display: flex;

  align-items: center;

  justify-content: space-between;

  gap: 20px;

  padding: 12px 16px;

  margin-bottom: 12px;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 13px;

  box-shadow: 0 5px 18px rgba(0, 0, 0, 0.025);
}

.switch-card > div {
  display: flex;

  flex-direction: column;
}

.switch-card strong {
  color: #34373d;

  font-size: 12px;
}

.switch-card small {
  margin-top: 3px;

  color: #999fa7;

  font-size: 10px;
}

/* =========================================================
   GUEST BOOK
========================================================= */

.guest-list {
  display: flex;

  flex-direction: column;

  gap: 10px;

  margin-top: 22px;
}

.guest-card {
  display: flex;

  align-items: flex-start;

  gap: 11px;

  padding: 14px;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 12px;
}

.guest-avatar {
  width: 32px;

  height: 32px;

  flex: 0 0 32px;

  display: flex;

  align-items: center;

  justify-content: center;

  border-radius: 9px;

  color: var(--primary);

  background: color-mix(in srgb, var(--primary) 7%, #fff);
}

.guest-content {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.guest-content strong {
  color: #33363b;

  font-size: 12px;
}

.guest-content p {
  margin: 5px 0;

  color: #666c74;

  font-size: 12px;

  line-height: 1.55;
}

.guest-content small {
  color: #a2a7ad;

  font-size: 9px;
}

/* =========================================================
   SETTINGS
========================================================= */

.settings-grid {
  display: grid;

  grid-template-columns: repeat(2, minmax(0, 1fr));

  gap: 11px;
}

.setting-item {
  min-width: 0;

  min-height: 66px;

  display: flex;

  align-items: center;

  justify-content: space-between;

  gap: 12px;

  padding: 10px 13px;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 12px;
}

.setting-info {
  min-width: 0;

  display: flex !important;

  flex-direction: row !important;

  align-items: center;

  gap: 10px;
}

.setting-icon {
  width: 32px;

  height: 32px;

  flex: 0 0 32px;

  display: flex;

  align-items: center;

  justify-content: center;

  border-radius: 9px;

  color: var(--primary);

  background: color-mix(in srgb, var(--primary) 7%, #fff);
}

.setting-info > div:last-child {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.setting-item strong {
  overflow: hidden;

  white-space: nowrap;

  text-overflow: ellipsis;

  color: #363940;

  font-size: 11px;
}

.setting-item small {
  margin-top: 3px;

  color: #999fa6;

  font-size: 9px;
}

/* =========================================================
   THEME
========================================================= */

.theme-current {
  display: flex;

  align-items: center;

  gap: 14px;

  padding: 15px;

  margin-bottom: 25px;

  background: #fff;

  border: 1px solid var(--border);

  border-radius: 15px;
}

.theme-color-preview {
  width: 54px;

  height: 54px;

  flex: 0 0 54px;

  display: flex;

  align-items: center;

  justify-content: center;

  border-radius: 13px;

  color: #fff;

  font-size: 22px;

  box-shadow: inset 0 0 0 1px rgba(255, 255, 255, 0.18);
}

.theme-current-info {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.theme-current-info span {
  color: #a0a5ac;

  font-size: 9px;
}

.theme-current-info strong {
  margin-top: 3px;

  color: #34373a;

  font-size: 14px;
}

.sub-heading {
  margin: 26px 0 14px;

  color: #3a3d42;

  font-size: 14px;

  font-weight: 700;
}

.color-grid {
  display: grid;

  grid-template-columns: repeat(3, minmax(0, 1fr));

  gap: 15px;
}

.color-field {
  min-width: 0;

  display: flex;

  flex-direction: column;

  gap: 7px;
}

.color-control {
  min-width: 0;

  display: flex;

  align-items: center;

  gap: 7px;
}

.color-control input[type="color"] {
  width: 40px;

  height: 40px;

  flex: 0 0 40px;

  padding: 2px;

  border: 1px solid #ddd;

  border-radius: 9px;

  background: #fff;

  cursor: pointer;
}

/* =========================================================
   PREVIEW
========================================================= */

.editor-preview {
  min-width: 0;

  min-height: 0;

  overflow: hidden;

  display: flex;

  flex-direction: column;

  background: #202124;

  border-left: 1px solid #161719;
}

.preview-toolbar {
  height: 55px;

  flex: 0 0 55px;

  display: flex;

  align-items: center;

  justify-content: space-between;

  gap: 15px;

  padding: 0 14px;

  color: #fff;

  background: #292a2d;

  border-bottom: 1px solid rgba(255, 255, 255, 0.05);
}

.preview-title {
  min-width: 0;

  display: flex;

  flex-direction: column;
}

.preview-title span {
  color: #858991;

  font-size: 7px;

  font-weight: 700;

  letter-spacing: 0.13em;
}

.preview-title strong {
  margin-top: 3px;

  overflow: hidden;

  white-space: nowrap;

  text-overflow: ellipsis;

  color: #fff;

  font-size: 11px;

  font-weight: 600;
}

.preview-device {
  display: flex;

  align-items: center;

  gap: 3px;

  padding: 3px;

  border-radius: 8px;

  background: #222326;
}

.preview-device button {
  width: 31px;

  height: 31px;

  display: flex;

  align-items: center;

  justify-content: center;

  border: 0;

  border-radius: 6px;

  background: transparent;

  color: #777b82;

  cursor: pointer;

  transition: color 0.2s ease, background 0.2s ease;
}

.preview-device button:hover {
  color: #ddd;
}

.preview-device button.active {
  color: #fff;

  background: #424449;
}

.preview-stage {
  flex: 1;

  min-height: 0;

  min-width: 0;

  overflow: auto;

  display: flex;

  justify-content: center;

  align-items: flex-start;

  padding: 28px 18px 35px;

  scrollbar-width: thin;
}

.preview-frame {
  min-width: 0;

  overflow: hidden;

  flex: 0 0 auto;

  background: #fff;

  box-shadow: 0 18px 60px rgba(0, 0, 0, 0.4);

  transition: width 0.3s ease, border-radius 0.3s ease;
}

.device-mobile .preview-frame {
  width: 375px;

  max-width: calc(100vw - 40px);

  min-height: 680px;

  border-radius: 27px;
}

.device-desktop .preview-frame {
  width: min(900px, 100%);

  min-height: 680px;

  border-radius: 4px;
}

/* =========================================================
   MOBILE BAR
========================================================= */

.mobile-editor-actions {
  display: none;
}

/* =========================================================
   MOBILE SHEET
========================================================= */

.mobile-menu-overlay {
  position: fixed;

  inset: 0;

  z-index: 2000;

  display: flex;

  align-items: flex-end;

  background: rgba(0, 0, 0, 0.42);

  backdrop-filter: blur(5px);
}

.mobile-menu-sheet {
  width: 100%;

  max-height: 85vh;

  overflow-y: auto;

  padding: 8px 14px calc(18px + env(safe-area-inset-bottom));

  background: #fff;

  border-radius: 22px 22px 0 0;

  box-shadow: 0 -20px 60px rgba(0, 0, 0, 0.2);
}

.sheet-handle {
  width: 38px;

  height: 4px;

  margin: 2px auto 15px;

  border-radius: 99px;

  background: #ddd;
}

.sheet-header {
  display: flex;

  align-items: center;

  justify-content: space-between;

  padding: 0 3px 13px;

  border-bottom: 1px solid #eee;
}

.sheet-header div {
  display: flex;

  flex-direction: column;
}

.sheet-header span {
  color: #a0a5ad;

  font-size: 8px;

  font-weight: 750;

  letter-spacing: 0.13em;
}

.sheet-header strong {
  margin-top: 3px;

  color: #2d3035;

  font-size: 17px;
}

.sheet-close {
  width: 35px;

  height: 35px;

  display: flex;

  align-items: center;

  justify-content: center;

  border: 0;

  border-radius: 10px;

  background: #f4f4f5;

  color: #777;

  cursor: pointer;
}

.mobile-menu-list {
  padding-top: 8px;
}

.mobile-menu-item {
  width: 100%;

  min-height: 57px;

  display: flex;

  align-items: center;

  gap: 11px;

  padding: 8px 7px;

  border: 0;

  border-radius: 12px;

  background: transparent;

  text-align: left;

  cursor: pointer;

  transition: background 0.2s ease;
}

.mobile-menu-item:hover,
.mobile-menu-item.active {
  background: color-mix(in srgb, var(--primary) 6%, #fff);
}

.mobile-menu-icon {
  width: 35px;

  height: 35px;

  flex: 0 0 35px;

  display: flex;

  align-items: center;

  justify-content: center;

  border-radius: 10px;

  color: #747980;

  background: #f5f6f7;
}

.mobile-menu-item.active .mobile-menu-icon {
  color: var(--primary);

  background: color-mix(in srgb, var(--primary) 10%, #fff);
}

.mobile-menu-item > span:nth-child(2) {
  min-width: 0;

  flex: 1;

  display: flex;

  flex-direction: column;
}

.mobile-menu-item strong {
  color: #3b3e44;

  font-size: 12px;
}

.mobile-menu-item small {
  margin-top: 2px;

  color: #9ca1a8;

  font-size: 9px;
}

.mobile-check {
  color: var(--primary);
}

.mobile-menu-divider {
  height: 1px;

  margin: 6px 5px;

  background: #eee;
}

/* =========================================================
   TOAST
========================================================= */

.save-toast {
  position: fixed;

  right: 24px;

  bottom: 24px;

  z-index: 3000;

  display: flex;

  align-items: center;

  gap: 9px;

  padding: 11px 14px;

  border-radius: 12px;

  color: #fff;

  background: #22252a;

  box-shadow: 0 15px 40px rgba(0, 0, 0, 0.2);

  font-size: 11px;

  font-weight: 600;
}

.save-toast.error {
  background: #a92828;
}

.toast-icon {
  display: flex;

  align-items: center;

  justify-content: center;
}

/* =========================================================
   TRANSITIONS
========================================================= */

.sheet-enter-active,
.sheet-leave-active {
  transition: opacity 0.25s ease;
}

.sheet-enter-active .mobile-menu-sheet,
.sheet-leave-active .mobile-menu-sheet {
  transition: transform 0.28s ease;
}

.sheet-enter-from,
.sheet-leave-to {
  opacity: 0;
}

.sheet-enter-from .mobile-menu-sheet,
.sheet-leave-to .mobile-menu-sheet {
  transform: translateY(100%);
}

.toast-enter-active,
.toast-leave-active {
  transition: opacity 0.2s ease, transform 0.2s ease;
}

.toast-enter-from,
.toast-leave-to {
  opacity: 0;

  transform: translateY(10px);
}

/* =========================================================
   LARGE DESKTOP
========================================================= */

@media (min-width: 1500px) {
  .editor-layout {
    grid-template-columns:
      255px
      minmax(620px, 1fr)
      520px;
  }

  .editor-center {
    padding: 38px 38px 80px;
  }
}

/* =========================================================
   LAPTOP
========================================================= */

@media (max-width: 1250px) {
  .editor-layout {
    grid-template-columns:
      220px
      minmax(0, 1fr)
      minmax(330px, 39vw);
  }

  .editor-center {
    padding: 28px 22px 70px;
  }

  .person-grid {
    grid-template-columns: 1fr;
  }

  .color-grid {
    grid-template-columns: repeat(2, minmax(0, 1fr));
  }
}

/* =========================================================
   TABLET
========================================================= */

@media (max-width: 1050px) {
  .editor-layout {
    grid-template-columns:
      210px
      minmax(0, 1fr);
  }

  /*
   * Không hiển thị preview bên phải
   * khi không đủ chiều rộng.
   *
   * Người dùng vẫn có thể nhấn
   * "Xem trước" để mở /preview.
   */
  .editor-preview {
    display: none;
  }

  .editor-center {
    width: 100%;

    max-width: 920px;

    margin: 0 auto;

    padding: 28px 25px 70px;
  }

  .gallery-editor {
    grid-template-columns: repeat(3, minmax(0, 1fr));
  }
}

/* =========================================================
   SMALL TABLET
========================================================= */

@media (max-width: 780px) {
  .editor-topbar {
    grid-template-columns:
      1fr
      auto;

    height: 62px;

    padding: 0 14px;
  }

  .editor-template {
    display: none;
  }

  .editor-actions {
    gap: 6px;
  }

  .top-button {
    width: 38px;

    padding: 0;
  }

  .top-button span {
    display: none;
  }

  .editor-layout {
    height: calc(100vh - 62px);

    display: block;

    padding-bottom: 66px;
  }

  .editor-sidebar {
    display: none;
  }

  .editor-center {
    width: 100%;

    height: 100%;

    padding: 23px 16px 80px;
  }

  .editor-panel {
    max-width: 100%;
  }

  .panel-header h1 {
    font-size: 27px;
  }

  .panel-header-row {
    align-items: flex-start;

    flex-direction: column;
  }

  .form-grid,
  .person-grid,
  .settings-grid {
    grid-template-columns: 1fr;
  }

  .editor-field.full {
    grid-column: auto;
  }

  .gallery-editor {
    grid-template-columns: repeat(2, minmax(0, 1fr));
  }

  .color-grid {
    grid-template-columns: repeat(2, minmax(0, 1fr));
  }

  .mobile-editor-actions {
    position: fixed;

    left: 0;

    right: 0;

    bottom: 0;

    z-index: 1500;

    height: calc(62px + env(safe-area-inset-bottom));

    display: grid;

    grid-template-columns: repeat(4, 1fr);

    padding-bottom: env(safe-area-inset-bottom);

    background: rgba(255, 255, 255, 0.95);

    border-top: 1px solid #e3e4e7;

    box-shadow: 0 -8px 25px rgba(0, 0, 0, 0.06);

    backdrop-filter: blur(18px);
  }

  .mobile-editor-actions button {
    min-width: 0;

    display: flex;

    flex-direction: column;

    align-items: center;

    justify-content: center;

    gap: 3px;

    border: 0;

    background: transparent;

    color: #8b9098;

    font-size: 9px;

    cursor: pointer;
  }

  .mobile-editor-actions button.active {
    color: var(--primary);
  }

  .mobile-editor-actions button:disabled {
    opacity: 0.5;
  }
}

/* =========================================================
   PHONE
========================================================= */

@media (max-width: 520px) {
  .editor-topbar {
    padding: 0 10px;
  }

  .brand-logo {
    width: 35px;

    height: 35px;

    flex-basis: 35px;
  }

  .brand-text strong {
    font-size: 13px;
  }

  .save-button {
    width: 38px;

    padding: 0;

    font-size: 0;
  }

  .save-button span {
    display: none;
  }

  .editor-center {
    padding: 20px 11px 78px;
  }

  .panel-header {
    margin-bottom: 22px;
  }

  .panel-header h1 {
    font-size: 25px;
  }

  .panel-header p {
    font-size: 11px;
  }

  .person-editor {
    padding: 15px;
  }

  .editor-card {
    padding: 15px;
  }

  .form-grid {
    gap: 13px;
  }

  .editor-field input,
  .editor-field textarea,
  .editor-field select {
    padding: 11px;

    font-size: 16px;
  }

  .gallery-editor {
    grid-template-columns: repeat(2, minmax(0, 1fr));

    gap: 9px;
  }

  .color-grid {
    grid-template-columns: 1fr;
  }

  .switch-card {
    padding: 10px 12px;
  }

  .save-toast {
    right: 12px;

    left: 12px;

    bottom: 75px;

    justify-content: center;
  }
}

/* =========================================================
   SMALL PHONE
========================================================= */

@media (max-width: 380px) {
  .editor-center {
    padding-left: 9px;

    padding-right: 9px;
  }

  .gallery-editor {
    grid-template-columns: 1fr;
  }

  .person-heading strong {
    font-size: 18px;
  }

  .mobile-editor-actions button {
    font-size: 8px;
  }
}

/* =========================================================
   REDUCED MOTION
========================================================= */

@media (prefers-reduced-motion: reduce) {
  .wedding-editor *,
  .wedding-editor *::before,
  .wedding-editor *::after {
    scroll-behavior: auto !important;

    transition-duration: 0.01ms !important;

    animation-duration: 0.01ms !important;

    animation-iteration-count: 1 !important;
  }
}
</style>