  ...
  <section class="sp-events" ref="sectionRef">
    <div class="sp-events__list">
      <article
        ...
        class="sp-event-card reveal"
        :style="{ '--delay': `${index * 120}ms` }"
      >
        <!-- EVENT TITLE -->
        <div class="sp-event-heading">
          <h2>{{ event.Title || "TIỆC CƯỚI" }}</h2>

          <div class="sp-mini-divider">
            <span></span>
            <i>❁</i>
            <span></span>
          </div>
        ...
        <!-- DATE -->
        <div v-if="event.hasDate" class="sp-event-date">
          <div class="sp-event-weekday">{{ event.weekday }}</div>

          <div class="sp-event-main-date">
            <div class="sp-date-side">
              <span>THÁNG</span>
              <strong>{{ event.month }}</strong>
            </div>

            <div class="sp-date-number">{{ event.day }}</div>
          ...
          <div v-if="event.lunar" class="sp-event-lunar">{{ event.lunar }}</div>
        </div>

        <!-- TIME -->
        <div v-if="event.time" class="sp-event-time">
          <div>
        ...
        <!-- SCHEDULE -->
        <div v-if="event.receptionTime || event.ceremonyTime" class="sp-event-schedule">
          <div v-if="event.receptionTime" class="sp-schedule-row">
            <div class="sp-schedule-dot"><span>❁</span></div>
          ...
          <div v-if="event.ceremonyTime" class="sp-schedule-row">
            <div class="sp-schedule-dot"><span>✧</span></div>
        ...
        <div v-if="event.date && event.calendarDays?.length" class="sp-calendar">
          ...
          <div class="sp-calendar__weekdays">
            <span>CN</span>
            <span>T2</span>
            <span>T3</span>
            <span>T4</span>
          ...
          <div class="sp-calendar__days">
            <div
              v-for="(day, dayIndex) in event.calendarDays"
              :key="dayIndex"
              class="sp-calendar__cell"
              :class="{ empty: !day, active: day === Number(event.day) }"
            >
              <template v-if="day">
                <div v-if="day === Number(event.day)" class="sp-active-day">
                  <span class="sp-active-bloom">❁</span>
                  <span>{{ day }}</span>
                </div>

                <span v-else class="sp-normal-day">{{ day }}</span>
              </template>
            </div>
          </div>
        ...
        <!-- RSVP -->
        <button type="button" class="sp-rsvp-btn" @click="openConfirmModal(event)">
          <span>❁</span>
          XÁC NHẬN THAM DỰ
          <span>❁</span>
        </button>
    ...
    <Teleport to="body">
      <Transition name="sp-modal">
        <div v-if="showConfirmModal" class="sp-confirm-overlay" @click.self="closeConfirmModal">
          <div class="sp-confirm-modal">
            <button type="button" class="sp-modal-close" @click="closeConfirmModal">×</button>

            <div class="sp-modal-header">
              <div class="sp-modal-symbol">❁</div>
            ...
            <!-- RECIPIENT -->
            <div v-if="hasRecipient" class="sp-recipient-box">
              <span>TRÂN TRỌNG KÍNH MỜI</span>
              <strong>{{ recipientName }}</strong>
            </div>

            <!-- NAME -->
            <div v-else class="sp-form-group">
            ...
            <div class="sp-form-group">
              ...
              <div class="sp-attendance">
                <button
                  type="button"
                  class="sp-attendance-option"
            ...
            <!-- PEOPLE -->
            <div v-if="form.attendance === 'attending'" class="sp-form-group">
              <label>Số người tham dự</label>

              <div class="sp-people-control">
            ...
            <div v-if="errorMessage" class="sp-form-error">{{ errorMessage }}</div>
            <div v-if="successMessage" class="sp-form-success">{{ successMessage }}</div>

            <button type="button" class="sp-modal-submit" :disabled="submitting" @click="submitConfirmation">
...
.sp-modal-submit:hover:not(:disabled) {
  transform: translateY(-2px);
}
...
.sp-modal-enter-active .sp-confirm-modal {
  animation: sp-modal-in 0.4s cubic-bezier(0.2, 0.8, 0.2, 1);
}
