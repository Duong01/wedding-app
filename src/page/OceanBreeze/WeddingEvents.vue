<template>
  <section class="ob-events" ref="sectionRef">
    <div class="ob-events__list">
      <article
        v-for="(event, index) in normalizedEvents"
        :key="event.Id || event.id || index"
        class="ob-event-card reveal"
        :style="{ '--delay': `${index * 120}ms` }"
      >
        <!-- EVENT TITLE -->
        <div class="ob-event-heading">
          <h2>{{ event.Title || "TIỆC CƯỚI" }}</h2>

          <div class="ob-mini-divider">
            <span></span>
            <i>〜</i>
            <span class="ob-mini-divider-flip"></span>
          </div>
        </div>

        <!-- DATE -->
        <div v-if="event.hasDate" class="ob-event-date">
          <div class="ob-event-weekday">{{ event.weekday }}</div>

          <div class="ob-event-main-date">
            <div class="ob-date-side">
              <span>THÁNG</span>
              <strong>{{ event.month }}</strong>
            </div>

            <div class="ob-date-number">{{ event.day }}</div>

            <div class="ob-date-side">
              <span>NĂM</span>
              <strong>{{ event.year }}</strong>
            </div>
          </div>

          <div v-if="event.lunar" class="ob-event-lunar">{{ event.lunar }}</div>
        </div>

        <!-- TIME -->
        <div v-if="event.time" class="ob-event-time">
          <div>
            <small>THỜI GIAN</small>
            <strong>{{ event.time }}</strong>
          </div>
        </div>

        <!-- SCHEDULE -->
        <div v-if="event.receptionTime || event.ceremonyTime" class="ob-event-schedule">
          <div v-if="event.receptionTime" class="ob-schedule-row">
            <div class="ob-schedule-dot"><span>〜</span></div>

            <div class="ob-schedule-content">
              <span>ĐÓN KHÁCH</span>
              <strong>{{ event.receptionTime }}</strong>
            </div>
          </div>

          <div v-if="event.ceremonyTime" class="ob-schedule-row">
            <div class="ob-schedule-dot"><span>❊</span></div>

            <div class="ob-schedule-content">
              <span>KHAI TIỆC</span>
              <strong>{{ event.ceremonyTime }}</strong>
            </div>
          </div>
        </div>

        <!-- CALENDAR -->
        <div v-if="event.date && event.calendarDays?.length" class="ob-calendar">
          <div class="ob-calendar__header">
            <span>LỊCH</span>
            <strong>THÁNG {{ event.month }} · {{ event.year }}</strong>
          </div>

          <div class="ob-calendar__weekdays">
            <span>CN</span>
            <span>T2</span>
            <span>T3</span>
            <span>T4</span>
            <span>T5</span>
            <span>T6</span>
            <span>T7</span>
          </div>

          <div class="ob-calendar__days">
            <div
              v-for="(day, dayIndex) in event.calendarDays"
              :key="dayIndex"
              class="ob-calendar__cell"
              :class="{ empty: !day, active: day === Number(event.day) }"
            >
              <template v-if="day">
                <div v-if="day === Number(event.day)" class="ob-active-day">
                  <span class="ob-active-wave">〜</span>
                  <span>{{ day }}</span>
                </div>

                <span v-else class="ob-normal-day">{{ day }}</span>
              </template>
            </div>
          </div>

          <a
            v-if="event.calendarUrl"
            :href="event.calendarUrl"
            target="_blank"
            rel="noopener noreferrer"
            class="ob-calendar-btn"
          >
            <span>＋</span>
            THÊM VÀO LỊCH
          </a>
        </div>

        <!-- RSVP -->
        <button type="button" class="ob-rsvp-btn" @click="openConfirmModal(event)">
          <span>〜</span>
          XÁC NHẬN THAM DỰ
          <span>❊</span>
        </button>

        <div class="ob-event-bottom">
          <span></span>
          <i>❊</i>
          <span class="ob-event-bottom-flip"></span>
        </div>
      </article>
    </div>

    <!-- RSVP MODAL -->
    <Teleport to="body">
      <Transition name="ob-modal">
        <div v-if="showConfirmModal" class="ob-confirm-overlay" @click.self="closeConfirmModal">
          <div class="ob-confirm-modal">
            <button type="button" class="ob-modal-close" @click="closeConfirmModal">×</button>

            <div class="ob-modal-header">
              <div class="ob-modal-symbol">〜</div>

              <span>THE CELEBRATION</span>

              <h3>Xác nhận tham dự</h3>

              <p>Sự hiện diện của bạn là niềm vui đối với gia đình chúng tôi.</p>
            </div>

            <!-- RECIPIENT -->
            <div v-if="hasRecipient" class="ob-recipient-box">
              <span>TRÂN TRỌNG KÍNH MỜI</span>
              <strong>{{ recipientName }}</strong>
            </div>

            <!-- NAME -->
            <div v-else class="ob-form-group">
              <label>Họ và tên</label>

              <input v-model.trim="form.name" type="text" maxlength="100" placeholder="Nhập tên của bạn" />
            </div>

            <!-- ATTENDANCE -->
            <div class="ob-form-group">
              <label>Bạn có tham dự không?</label>

              <div class="ob-attendance">
                <button
                  type="button"
                  class="ob-attendance-option"
                  :class="{ selected: form.attendance === 'attending' }"
                  @click="form.attendance = 'attending'"
                >
                  <span>✓</span>
                  Có, tôi sẽ tham dự
                </button>

                <button
                  type="button"
                  class="ob-attendance-option"
                  :class="{ selected: form.attendance === 'not_attending' }"
                  @click="form.attendance = 'not_attending'"
                >
                  <span>×</span>
                  Rất tiếc, tôi không thể tham dự
                </button>
              </div>
            </div>

            <!-- PEOPLE -->
            <div v-if="form.attendance === 'attending'" class="ob-form-group">
              <label>Số người tham dự</label>

              <div class="ob-people-control">
                <button type="button" @click="decreasePeople">−</button>
                <strong>{{ form.numberOfPeople }}</strong>
                <button type="button" @click="increasePeople">+</button>
              </div>
            </div>

            <div v-if="errorMessage" class="ob-form-error">{{ errorMessage }}</div>
            <div v-if="successMessage" class="ob-form-success">{{ successMessage }}</div>

            <button type="button" class="ob-modal-submit" :disabled="submitting" @click="submitConfirmation">
              {{ submitting ? "ĐANG GỬI..." : "GỬI XÁC NHẬN" }}
            </button>
          </div>
        </div>
      </Transition>
    </Teleport>
  </section>
</template>

<script setup>
import { computed, onMounted, onBeforeUnmount, ref } from "vue";
import dayjs from "dayjs";
import { useRoute } from "vue-router";
import { Confirm } from "@/model/api";

const props = defineProps({
  events: { type: Array, default: () => [] },
  recipientName: { type: [Object, Array, String], default: null },
});

const route = useRoute();

const sectionRef = ref(null);

const showConfirmModal = ref(false);
const selectedEvent = ref(null);

const form = ref({
  name: "",
  attendance: "",
  numberOfPeople: 1,
});

const submitting = ref(false);
const errorMessage = ref("");
const successMessage = ref("");

const recipientName = computed(() => {
  const value = props.recipientName;

  if (!value) return "";

  if (typeof value === "string") return value.trim();

  if (Array.isArray(value)) return value[0]?.Name || "";

  return value.Name || "";
});

const hasRecipient = computed(() => !!recipientName.value);

/* =========================================
   NORMALIZE EVENTS
========================================= */

const normalizedEvents = computed(() => {
  return (props.events || []).map((item) => {
    const rawDate = item.EventDate || item.Date || item.StartDate;

    const date = dayjs(rawDate);

    let month = "";
    let day = "";
    let year = "";
    let weekday = "";

    if (date.isValid()) {
      day = date.format("DD");
      month = date.format("MM");
      year = date.format("YYYY");

      const weekdays = ["CHỦ NHẬT", "THỨ HAI", "THỨ BA", "THỨ TƯ", "THỨ NĂM", "THỨ SÁU", "THỨ BẢY"];

      weekday = weekdays[date.day()];
    }

    const calendarDays = item.calendarDays || buildCalendarDays(Number(year), Number(month));

    return {
      ...item,

      hasDate: date.isValid(),

      time:
        item.EventTime ||
        item.Time ||
        item.StartTime ||
        (date.isValid() ? date.format("HH:mm") : ""),

      day,
      month,
      year,
      weekday,

      date: rawDate,

      lunar: item.LunarDate || item.lunar || "",

      receptionTime: item.ReceptionTime || item.receptionTime || "",

      ceremonyTime: item.CeremonyTime || item.ceremonyTime || "",

      location: item.Location || item.location || "",

      address: item.Address || item.address || "",

      map: item.Map || item.map || "",

      rsvpUrl: item.RsvpUrl || item.rsvpUrl || item.RSVPUrl || "",

      calendarUrl: item.CalendarUrl || item.calendarUrl || "",

      calendarDays,
    };
  });
});

/* =========================================
   CALENDAR
========================================= */

function buildCalendarDays(year, month) {
  if (!year || !month) return [];

  const firstDay = dayjs(`${year}-${String(month).padStart(2, "0")}-01`);

  const daysInMonth = firstDay.daysInMonth();

  const startDay = firstDay.day();

  const result = [];

  for (let i = 0; i < startDay; i++) {
    result.push(null);
  }

  for (let i = 1; i <= daysInMonth; i++) {
    result.push(i);
  }

  return result;
}

/* =========================================
   RSVP
========================================= */

function openConfirmModal(event) {
  selectedEvent.value = event;

  form.value = {
    name: recipientName.value || "",
    attendance: "attending",
    numberOfPeople: 1,
  };

  errorMessage.value = "";
  successMessage.value = "";

  showConfirmModal.value = true;

  document.body.style.overflow = "hidden";
}

function closeConfirmModal() {
  showConfirmModal.value = false;

  document.body.style.overflow = "";
}

function increasePeople() {
  if (form.value.numberOfPeople < 10) {
    form.value.numberOfPeople++;
  }
}

function decreasePeople() {
  if (form.value.numberOfPeople > 1) {
    form.value.numberOfPeople--;
  }
}

async function submitConfirmation() {
  errorMessage.value = "";
  successMessage.value = "";

  if (!hasRecipient.value && !form.value.name) {
    errorMessage.value = "Vui lòng nhập họ và tên.";
    return;
  }

  if (!form.value.attendance) {
    errorMessage.value = "Vui lòng chọn xác nhận tham dự.";
    return;
  }

  submitting.value = true;

  try {
    const slug = route.params.slug
      ? route.params.token
        ? `${route.params.slug}/${route.params.token}`
        : route.params.slug
      : "";

    const payload = {
      Slug: slug,
      RecipientToken: route.params.token || null,
      GuestName: form.value.name,
      Attendance:
        form.value.attendance === "attending" ? "Có tham dự" : "Không tham dự",
      NumberOfPeople:
        form.value.attendance === "attending" ? form.value.numberOfPeople : 0,
    };

    const response = await Confirm(payload);

    const result = response?.data;

    if (!result || result.status !== "success") {
      throw new Error(result?.message || "Không thể gửi xác nhận.");
    }

    successMessage.value = "Cảm ơn bạn đã xác nhận tham dự ❤️";

    setTimeout(() => {
      closeConfirmModal();
    }, 1800);
  } catch (error) {
    errorMessage.value =
      error?.response?.data?.message ||
      error?.message ||
      "Có lỗi xảy ra. Vui lòng thử lại.";
  } finally {
    submitting.value = false;
  }
}

/* =========================================
   SCROLL REVEAL
========================================= */

let observer;

onMounted(() => {
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          entry.target.classList.add("visible");
          observer.unobserve(entry.target);
        }
      });
    },
    { threshold: 0.12 }
  );

  sectionRef.value?.querySelectorAll(".reveal").forEach((el) => observer.observe(el));
});

onBeforeUnmount(() => {
  observer?.disconnect();

  document.body.style.overflow = "";
});
</script>

<style scoped>
.ob-events {
  position: relative;

  width: 100%;

  overflow: hidden;
}

/* =====================================================
   EVENTS LIST
===================================================== */

.ob-events__list {
  width: min(100%, 680px);
  margin: 0 auto;

  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 34px;
}

/* =====================================================
   EVENT CARD
===================================================== */

.ob-event-card {
  position: relative;

  width: 100%;
  max-width: 520px;

  padding: 30px 24px 26px;

  display: flex;
  flex-direction: column;
  align-items: center;

  text-align: center;

  color: #274b5c;

  border: 1px solid rgba(74, 154, 184, 0.35);
  border-radius: 24px;

  background: linear-gradient(170deg, rgba(255, 255, 255, 0.94), rgba(244, 250, 252, 0.88));

  box-shadow: 0 18px 44px rgba(29, 106, 142, 0.1);

  overflow: hidden;
}

.ob-event-card::before {
  content: "";
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  height: 8px;

  background:
    radial-gradient(circle at 12px 8px, transparent 9px, rgba(168, 208, 220, 0.6) 10px, transparent 11px) repeat-x;
  background-size: 24px 10px;
  background-position: 0 0;

  pointer-events: none;
}

/* =====================================================
   HEADING
===================================================== */

.ob-event-heading {
  width: 100%;
  text-align: center;
}

.ob-event-heading h2 {
  margin: 0 0 8px;

  font-family: "Cormorant Garamond", Georgia, serif;

  font-size: 30px;
  font-weight: 600;

  letter-spacing: 0.04em;

  color: #1d6a8e;
}

.ob-mini-divider {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 8px;

  color: #e8927c;
}

.ob-mini-divider span {
  width: 35px;
  height: 1px;

  background: linear-gradient(90deg, transparent, rgba(74, 154, 184, 0.7));
}

.ob-mini-divider-flip {
  transform: rotate(180deg);
}

.ob-mini-divider i {
  font-size: 12px;
  font-style: normal;
}

/* =====================================================
   DATE
===================================================== */

.ob-event-date {
  width: 100%;
  text-align: center;
}

.ob-event-weekday {
  margin-top: 14px;

  font-size: 10px;
  font-weight: 700;

  letter-spacing: 0.24em;

  color: #4a9ab8;
}

.ob-event-main-date {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 22px;

  margin: 8px 0;
}

.ob-date-number {
  font-family: "Cormorant Garamond", Georgia, serif;

  font-size: clamp(44px, 13vw, 76px);
  font-weight: 600;

  line-height: 0.85;

  color: #1d6a8e;
}

.ob-date-side {
  display: flex;
  flex-direction: column;
  gap: 3px;

  font-size: 8px;

  letter-spacing: 0.14em;

  color: #e8927c;
}

.ob-date-side strong {
  font-size: 18px;
  font-weight: 600;

  color: #274b5c;
}

.ob-event-lunar {
  font-size: 14px;
  font-style: italic;

  color: #5b8296;
}

/* =====================================================
   TIME
===================================================== */

.ob-event-time {
  width: 100%;

  display: flex;
  align-items: center;
  justify-content: center;

  margin: 10px auto 0;
  padding: 15px 0;

  border-bottom: 1px solid rgba(74, 154, 184, 0.3);
}

.ob-event-time div {
  display: flex;
  flex-direction: column;
  align-items: center;
  text-align: center;
}

.ob-event-time small {
  font-size: 8px;

  letter-spacing: 0.2em;

  color: #4a9ab8;
}

.ob-event-time strong {
  font-size: 23px;
  font-weight: 600;

  color: #1d6a8e;
}

/* =====================================================
   SCHEDULE
===================================================== */

.ob-event-schedule {
  position: relative;

  width: min(100%, 380px);

  margin: 20px auto 0;
  padding-left: 28px;

  text-align: left;
}

.ob-event-schedule::before {
  content: "";
  position: absolute;

  left: 6px;
  top: 12px;
  bottom: 12px;

  width: 1px;

  background: rgba(74, 154, 184, 0.45);
}

.ob-schedule-row {
  position: relative;

  display: flex;
  align-items: center;
  gap: 16px;

  min-height: 42px;
}

.ob-schedule-dot {
  position: absolute;
  left: -28px;

  width: 13px;
  height: 13px;

  display: flex;
  align-items: center;
  justify-content: center;

  border-radius: 50%;
  border: 1px solid #4a9ab8;

  background: #f4fafc;

  z-index: 2;
}

.ob-schedule-dot span {
  font-size: 7px;

  color: #e8927c;
}

.ob-schedule-content {
  width: 100%;

  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 20px;
}

.ob-schedule-content span {
  font-size: 9px;

  letter-spacing: 0.16em;

  color: #5b8296;
}

.ob-schedule-content strong {
  font-size: 21px;
  font-weight: 600;

  color: #1d6a8e;
}

/* =====================================================
   CALENDAR
===================================================== */

.ob-calendar {
  width: min(100%, 400px);

  margin: 25px auto 0;

  text-align: center;
}

.ob-calendar__header {
  width: 100%;

  display: flex;
  align-items: center;
  justify-content: space-between;

  margin-bottom: 12px;
}

.ob-calendar__header span {
  font-size: 9px;
  font-weight: 700;

  letter-spacing: 0.2em;

  color: #4a9ab8;
}

.ob-calendar__header strong {
  font-size: 9px;

  letter-spacing: 0.1em;

  color: #274b5c;
}

.ob-calendar__weekdays,
.ob-calendar__days {
  width: 100%;

  display: grid;
  grid-template-columns: repeat(7, 1fr);

  text-align: center;
}

.ob-calendar__weekdays {
  padding-bottom: 8px;

  border-bottom: 1px solid rgba(74, 154, 184, 0.25);
}

.ob-calendar__weekdays span {
  font-size: 8px;
  font-weight: 700;

  color: #4a9ab8;
}

.ob-calendar__cell {
  min-height: 31px;

  display: flex;
  align-items: center;
  justify-content: center;
}

.ob-normal-day {
  font-size: 12px;

  color: #4f7488;
}

.ob-active-day {
  position: relative;

  width: 38px;
  height: 38px;

  display: flex;
  align-items: center;
  justify-content: center;
}

.ob-active-wave {
  position: absolute;

  font-size: 31px;

  color: #4a9ab8;
}

.ob-active-day span:last-child {
  position: relative;

  font-weight: 700;

  color: #ffffff;
}

.ob-calendar-btn {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: 6px;

  margin-top: 15px;

  font-size: 9px;
  font-weight: 700;

  letter-spacing: 0.18em;

  text-decoration: none;

  color: #e8927c;
}

/* =====================================================
   RSVP
===================================================== */

.ob-rsvp-btn {
  width: min(100%, 400px);

  margin: 25px auto 0;

  display: flex;
  align-items: center;
  justify-content: center;
  gap: 13px;

  padding: 13px 18px;

  border: 0;
  border-radius: 999px;

  color: #ffffff;

  background: linear-gradient(135deg, #4a9ab8, #1d6a8e);

  box-shadow: 0 10px 24px rgba(29, 106, 142, 0.26);

  font-size: 9px;
  font-weight: 700;

  letter-spacing: 0.2em;

  cursor: pointer;

  transition: transform 0.25s ease, box-shadow 0.25s ease;
}

.ob-rsvp-btn:hover {
  transform: translateY(-2px);

  box-shadow: 0 14px 30px rgba(29, 106, 142, 0.34);
}

/* =====================================================
   BOTTOM ORNAMENT
===================================================== */

.ob-event-bottom {
  width: 100%;

  margin-top: 30px;

  display: flex;
  align-items: center;
  justify-content: center;
  gap: 12px;

  color: #4a9ab8;
}

.ob-event-bottom span {
  width: 60px;
  height: 1px;

  background: linear-gradient(90deg, transparent, rgba(74, 154, 184, 0.6));
}

.ob-event-bottom-flip {
  transform: rotate(180deg);
}

.ob-event-bottom i {
  font-size: 12px;
  font-style: normal;
}

/* =====================================================
   REVEAL
===================================================== */

.reveal {
  opacity: 0;

  transform: translateY(35px);

  transition:
    opacity 0.8s ease var(--delay, 0ms),
    transform 0.8s cubic-bezier(0.2, 0.8, 0.2, 1) var(--delay, 0ms);
}

.reveal.visible {
  opacity: 1;

  transform: translateY(0);
}

/* =====================================================
   MODAL
===================================================== */

.ob-confirm-overlay {
  position: fixed;
  inset: 0;
  z-index: 99999;

  display: flex;
  align-items: center;
  justify-content: center;

  padding: 20px;

  background: rgba(23, 74, 96, 0.45);

  backdrop-filter: blur(6px);
}

.ob-confirm-modal {
  position: relative;

  width: min(100%, 470px);

  max-height: 90vh;

  overflow-y: auto;

  padding: 40px 30px;

  border: 1px solid rgba(74, 154, 184, 0.5);
  border-radius: 24px;

  background: linear-gradient(170deg, #ffffff, #f4fafc);

  box-shadow: 0 30px 80px rgba(23, 74, 96, 0.3);

  text-align: center;

  color: #274b5c;
}

.ob-modal-close {
  position: absolute;
  top: 12px;
  right: 15px;

  width: 35px;
  height: 35px;

  border: 0;

  font-size: 27px;

  color: #4a9ab8;

  background: transparent;

  cursor: pointer;
}

.ob-modal-header {
  text-align: center;
}

.ob-modal-symbol {
  font-size: 20px;

  margin-bottom: 8px;

  color: #e8927c;
}

.ob-modal-header > span {
  font-size: 8px;

  letter-spacing: 0.28em;

  color: #4a9ab8;
}

.ob-modal-header h3 {
  margin: 8px 0;

  font-family: "Cormorant Garamond", Georgia, serif;

  font-size: 32px;
  font-weight: 600;

  color: #1d6a8e;
}

.ob-modal-header p {
  margin: 0;

  font-size: 14px;

  color: #4f7488;
}

/* =====================================================
   FORM
===================================================== */

.ob-form-group {
  margin-top: 20px;

  text-align: left;
}

.ob-form-group label {
  display: block;

  margin-bottom: 8px;

  font-size: 10px;
  font-weight: 700;

  letter-spacing: 0.12em;

  color: #4a9ab8;
}

.ob-form-group input {
  width: 100%;

  padding: 13px 14px;

  border: 1px solid rgba(74, 154, 184, 0.4);
  border-radius: 12px;

  outline: none;

  font-size: 16px;

  color: #274b5c;

  background: rgba(255, 255, 255, 0.9);

  transition: border-color 0.2s ease, box-shadow 0.2s ease;
}

.ob-form-group input:focus {
  border-color: #4a9ab8;

  box-shadow: 0 0 0 3px rgba(74, 154, 184, 0.16);
}

.ob-attendance {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.ob-attendance-option {
  padding: 12px 14px;

  border: 1px solid rgba(74, 154, 184, 0.4);
  border-radius: 12px;

  text-align: left;

  font-size: 13px;

  color: #274b5c;

  background: rgba(255, 255, 255, 0.8);

  cursor: pointer;

  transition: all 0.25s ease;
}

.ob-attendance-option.selected {
  border-color: #1d6a8e;

  background: rgba(74, 154, 184, 0.14);

  font-weight: 600;
}

.ob-attendance-option span {
  margin-right: 8px;

  color: #e8927c;
}

.ob-people-control {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 25px;
}

.ob-people-control button {
  width: 38px;
  height: 38px;

  border: 1px solid #4a9ab8;
  border-radius: 50%;

  font-size: 20px;

  color: #274b5c;

  background: #f4fafc;

  cursor: pointer;
}

.ob-people-control strong {
  min-width: 25px;

  text-align: center;

  font-size: 18px;

  color: #274b5c;
}

.ob-recipient-box {
  margin: 20px 0;

  padding: 18px;

  border: 1px dashed rgba(74, 154, 184, 0.5);
  border-radius: 16px;

  text-align: center;

  background: rgba(226, 241, 245, 0.6);
}

.ob-recipient-box span {
  display: block;

  font-size: 8px;

  letter-spacing: 0.22em;

  color: #4a9ab8;
}

.ob-recipient-box strong {
  display: block;
  margin-top: 6px;

  font-family: "Allura", cursive;
  font-size: 28px;
  font-weight: 400;

  color: #1d6a8e;
}

.ob-form-error,
.ob-form-success {
  margin-top: 15px;

  padding: 10px;

  border-radius: 10px;

  text-align: center;

  font-size: 12px;
}

.ob-form-error {
  color: #a34d4d;

  background: rgba(163, 77, 77, 0.08);
}

.ob-form-success {
  color: #2e7d6b;

  background: rgba(46, 125, 107, 0.08);
}

.ob-modal-submit {
  width: 100%;

  margin-top: 22px;

  padding: 14px;

  border: 0;
  border-radius: 999px;

  font-size: 10px;
  font-weight: 700;

  letter-spacing: 0.2em;

  color: #ffffff;

  background: linear-gradient(135deg, #4a9ab8, #1d6a8e);

  box-shadow: 0 10px 24px rgba(29, 106, 142, 0.26);

  cursor: pointer;

  transition: transform 0.25s ease;
}

.ob-modal-submit:hover:not(:disabled) {
  transform: translateY(-2px);
}

.ob-modal-submit:disabled {
  opacity: 0.6;

  cursor: not-allowed;
}

/* =====================================================
   MODAL ANIMATION
===================================================== */

.ob-modal-enter-active,
.ob-modal-leave-active {
  transition: opacity 0.3s ease;
}

.ob-modal-enter-from,
.ob-modal-leave-to {
  opacity: 0;
}

.ob-modal-enter-active .ob-confirm-modal {
  animation: ob-modal-in 0.4s cubic-bezier(0.2, 0.8, 0.2, 1);
}

@keyframes ob-modal-in {
  from {
    opacity: 0;
    transform: translateY(30px) scale(0.96);
  }

  to {
    opacity: 1;
    transform: translateY(0) scale(1);
  }
}

/* =====================================================
   MOBILE
===================================================== */

@media (max-width: 600px) {
  .ob-events__list {
    width: 100%;
    padding: 0 18px;
    gap: 25px;
  }

  .ob-event-card {
    max-width: 430px;

    padding: 26px 18px 22px;
  }

  .ob-event-heading h2 {
    font-size: 26px;
  }

  .ob-event-main-date {
    gap: 14px;
  }

  .ob-date-number {
    font-size: 62px;
  }

  .ob-date-side strong {
    font-size: 16px;
  }

  .ob-event-schedule {
    width: min(100%, 340px);
  }

  .ob-schedule-content strong {
    font-size: 19px;
  }

  .ob-calendar {
    width: min(100%, 360px);
  }

  .ob-rsvp-btn {
    width: min(100%, 360px);
  }

  .ob-confirm-modal {
    padding: 35px 20px 25px;
  }
}

/* =====================================================
   REDUCE MOTION
===================================================== */

@media (prefers-reduced-motion: reduce) {
  .reveal {
    opacity: 1;

    transform: none;

    transition: none;
  }

  .ob-rsvp-btn,
  .ob-modal-submit {
    transition: none;
  }
}
</style>
