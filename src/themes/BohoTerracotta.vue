  ...
  <div class="boho-terracotta-theme">
    ...
    <main v-else class="bt-invitation">
      ...
      <div class="bt-content">
        <section v-if="showCouple" class="bt-section"><WeddingCouple :wedding="wedding" :guest-name="guestName" /></section>
        <section v-if="showStory && wedding?.story" class="bt-section"><WeddingStory :story="wedding.story" /></section>
        <section v-if="showEvents && events.length" class="bt-section"><WeddingEvents :events="events" :recipient-name="wedding?.recipientName" /></section>
        <section v-if="showTimeline && timeline.length" class="bt-section"><Timeline :timeline="timeline" :events="events" /></section>
        <section v-if="showCountdown" class="bt-section"><WeddingCountdown :countdown="countdownTarget" :wedding-date="wedding?.weddingDate" /></section>
        <section v-if="showGallery && gallery.length" class="bt-section"><WeddingGallery :gallery="gallery" /></section>
      </div>

      <section v-if="showMap && events.length" class="bt-section"><WeddingMap :events="events" /></section>
      <section v-if="showGift && gifts.length" class="bt-section"><WeddingGifts :gifts="gifts" /></section>
      <section v-if="showGuestBook" class="bt-section"><WeddingWishes :wishes="wishes" :wedding="wedding" /></section>
...
.boho-terracotta-theme {
  --theme-primary: #9c5b3f;
  --theme-secondary: #c97b5d;
  --theme-accent: #d9b08c;
  --theme-bg: #faf3ec;
  ...
  width: 100%;
  background:
    repeating-linear-gradient(45deg, rgba(156, 91, 63, 0.025) 0 1px, transparent 1px 12px),
    repeating-linear-gradient(-45deg, rgba(156, 91, 63, 0.025) 0 1px, transparent 1px 12px),
    var(--theme-bg);
...
.bt-invitation { width: 100%; }

.bt-content {
  padding: 0 20px 30px;
  background: linear-gradient(180deg, rgba(255, 255, 255, 0.2), rgba(242, 226, 208, 0.6));
}

.bt-section { max-width: 1100px; margin: 0 auto 22px; }
</style>
